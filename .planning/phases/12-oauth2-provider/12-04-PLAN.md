...
must_haves:
  truths:
    ...
    - "Sessions and revoke routes support dual auth: GPT Bearer token OR x-gpt-clerk-user-id header"
...
Handler logic:
- Use dual-auth resolution helper (see below)
...
**Dual-auth resolution helper** (implement as a local function in routes.ts):
The sessions and revoke endpoints are called from TWO contexts:
1. ChatGPT (sends GPT Bearer token `gpt_at_...`)
2. Candidate profile page (sends `x-gpt-clerk-user-id` header via api-gateway)

Create a `resolveClerkUserId(request, oauthService)` helper:
- If Authorization header contains `Bearer gpt_at_...`: validate via `oauthService.validateAccessToken()`, return `clerkUserId` from token claims
- Else if `x-gpt-clerk-user-id` header is present: return that header value directly (trusted -- behind gateway)
- Else: reply 401 with `{ error: 'authentication_required' }`

This pattern is safe because gpt-service sits behind the api-gateway; the `x-gpt-clerk-user-id` header is only set by the candidate profile page which is itself authenticated via Clerk at the frontend layer.
...
6. resolveClerkUserId helper supports both GPT Bearer token and x-gpt-clerk-user-id header
7. Sessions and revoke routes use resolveClerkUserId for dual-auth
  </verify>
  <done>OAuth endpoints handle authorize, token exchange, refresh, revoke, session listing, and consent check. Token endpoint supports both authorization_code and refresh_token grant types. Sessions and revoke endpoints support dual auth (GPT token or x-gpt-clerk-user-id header) for use by both ChatGPT and the candidate profile page.</done>
</task>
