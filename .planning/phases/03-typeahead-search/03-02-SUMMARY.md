...
<label className="input input-sm w-full">
  ...
  {query && <button className="btn btn-ghost btn-xs btn-circle"><i className="fa-xmark" /></button>}
...
1. **DaisyUI v5 input pattern:** Use `<label class="input">` as flex container with icon + `input.grow` as siblings. Never use absolute positioning for input icons in DaisyUI.
