...
export const COMMISSION_RATES: Record<Tier, CommissionRates> = {
    free: {
        candidate_recruiter: 0.2,
        job_owner: 0.1,
        company_recruiter: 0.1,
        candidate_sourcer: 0.06,
        company_sourcer: 0.06,
    },
    paid: {
        candidate_recruiter: 0.3,
        job_owner: 0.15,
    ...
    premium: {
        candidate_recruiter: 0.4,
        job_owner: 0.2,
        company_recruiter: 0.2,
        candidate_sourcer: 0.1,
        company_sourcer: 0.1,
    },
};
...
export function useCalculator(initialState?: Partial<CalculatorState>) {
    ...
    const upgradeValue = useMemo(() => {
        ...
        const premiumPayout =
            payouts.find((p) => p.tier === "premium")?.payout ?? 0;
    ...
    const setFeePercentage = (percentage: number) => {
        setState((prev) => ({
            ...prev,
            feePercentage: Math.min(100, Math.max(0, percentage)),
        }));
    };
