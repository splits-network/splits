...
export function SupportThread({ conversation, onConversationUpdate }: SupportThreadProps) {
    ...
    const fetchMessages = useCallback(async () => {
        try {
            ...
            const res = await fetch(
                `${GATEWAY_URL}/api/v3/support/admin/support/conversations/${conversation.id}/messages?limit=100`,
    ...
    const handleSend = async () => {
        ...
        try {
            const token = await getToken().catch(() => null);
            const res = await fetch(
                `${GATEWAY_URL}/api/v3/support/admin/support/conversations/${conversation.id}/messages`,
                {
                    method: 'POST',
                    headers: {
    ...
    const handleStatusChange = async (status: string) => {
        try {
            const token = await getToken().catch(() => null);
            await fetch(
                `${GATEWAY_URL}/api/v3/support/admin/support/conversations/${conversation.id}`,
                {
                    method: 'PATCH',
                    headers: {
                        'Content-Type': 'application/json',
    ...
    const handleClaim = async () => {
        try {
            const token = await getToken().catch(() => null);
            await fetch(
                `${GATEWAY_URL}/api/v3/support/admin/support/conversations/${conversation.id}/claim`,
                {
                    method: 'POST',
                    headers: { Authorization: `Bearer ${token}` },
                },
