...
import { SAMPLE_FIRM_PROFILE, type FirmProfileData } from "./profile-data";
import { firmInitials } from "./data";

/* ─── Sub-components ─────────────────────────────────────────────────────── */

function HeroHeader({ firm }: { firm: FirmProfileData }) {
...
function AboutPanel({ firm }: { firm: FirmProfileData }) {
    return (
        <div className="space-y-10">
            ...
            {/* Tagline highlight */}
            {firm.tagline && (
                <div className="bg-primary/5 border border-primary/10 px-6 py-5">
            ...
            {/* Recent Placements */}
            {firm.recentPlacements.length > 0 && (
                <div>
                    <p className="text-xs font-bold uppercase tracking-[0.22em] text-base-content/30 mb-4">
                        Recent Placements
                    </p>
...
function SpecialtiesPanel({ firm }: { firm: FirmProfileData }) {
    return (
        <div className="space-y-10">
            {/* Industries */}
...
function TeamPanel({ firm }: { firm: FirmProfileData }) {
    return (
        <div>
...
function ReviewsPanel({ firm }: { firm: FirmProfileData }) {
    return (
        <div>
            <p className="text-xs font-bold uppercase tracking-[0.22em] text-base-content/30 mb-8">
                Client Testimonials
...
export default function FirmProfileEditorial() {
    const firm = SAMPLE_FIRM_PROFILE;
    const [activeTab, setActiveTab] = useState<TabKey>("about");

    return (
        <main className="min-h-screen bg-base-100">
            <HeroHeader firm={firm} />
