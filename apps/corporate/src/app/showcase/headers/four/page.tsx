        ...
        >
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div className="flex items-center justify-between h-18 lg:h-20">
                    {/* Logo */}
                    <a href="#" className="cin-header-logo flex items-center gap-3 group">
                        ...
                        {NAV_ITEMS.map((item) => (
                            <div key={item.label} className="relative">
                                ...
                                >
                                    {item.label}
                                    {item.children && (
                                        <i className={`fa-regular fa-chevron-down text-[9px] transition-transform duration-200 ${activeDropdown === item.label ? "rotate-180" : ""}`} />
                                    )}
                                </button>
                                ...
                                {item.children && activeDropdown === item.label && (
                                    ...
                                    >
                                            ...
                                            {item.children.map((child) => (
                                                ...
                                                >
                                                    <div className="w-9 h-9 rounded-lg bg-white/[0.04] flex items-center justify-center flex-shrink-0 group-hover/item:bg-primary/15 transition-colors">
                                                        <i className={`${child.icon} text-white/40 text-sm group-hover/item:text-primary transition-colors`} />
                                                    </div>
                                                    <div>
                                        ...
                                        <div className="border-t border-white/[0.06] px-4 py-3 bg-white/[0.02]">
                                            <a href="#" className="text-xs text-primary/70 hover:text-primary font-medium flex items-center gap-1.5 transition-colors">
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <div className="relative">
                            <button
                                ...
                                className={`cin-header-action btn btn-ghost btn-sm btn-square transition-all duration-200 ${
                                    searchOpen ? "text-primary bg-primary/10" : "text-white/40 hover:text-white hover:bg-white/[0.06]"
                                }`}
                            >
                                ...
                                <div className="absolute top-full right-0 mt-2 w-80 bg-neutral border border-white/[0.08] rounded-xl shadow-2xl shadow-black/40 p-3">
                                    <div className="relative">
                                        ...
                                        <input
                                            ...
                                            autoFocus
                                            className="input input-sm w-full pl-9 bg-white/[0.04] border-white/[0.08] text-white placeholder:text-white/30 focus:border-primary/40"
                                        />
                                    </div>
                                    <div className="mt-2 px-1">
                                        <p className="text-[10px] text-white/20 uppercase tracking-widest mb-2">Quick links</p>
                                        {["Active Jobs", "My Candidates", "Placements"].map((link) => (
                                            <a key={link} href="#" className="block text-xs text-white/40 hover:text-white py-1.5 transition-colors">
                                                <i className="fa-regular fa-arrow-right text-[9px] mr-2 text-white/20" />{link}
                                            </a>
                                        ))}
                                    </div>
                                </div>
                        ...
                        <div className="relative hidden sm:block">
                            ...
                            >
                                <div className="w-7 h-7 rounded-full bg-primary/20 flex items-center justify-center text-[10px] font-bold text-primary">MC</div>
                                <span className="text-white/60 text-sm font-medium hidden md:inline">Marcus</span>
                                <i className={`fa-regular fa-chevron-down text-[9px] text-white/30 transition-transform duration-200 ${userMenuOpen ? "rotate-180" : ""}`} />
                            </button>
                            {userMenuOpen && (
                                <div className="absolute top-full right-0 mt-2 w-56 bg-neutral border border-white/[0.08] rounded-xl shadow-2xl shadow-black/40 overflow-hidden">
                                    <div className="p-3 border-b border-white/[0.06]">
                                        <p className="text-white/80 text-sm font-semibold">Marcus Chen</p>
                                        <p className="text-white/30 text-xs">marcus@recruiter.io</p>
                                    </div>
                                    <div className="p-1.5">
                                        {[
                                            { icon: "fa-regular fa-user", label: "Profile" },
                                            { icon: "fa-regular fa-gear", label: "Settings" },
                                            { icon: "fa-regular fa-credit-card", label: "Billing" },
                                            { icon: "fa-regular fa-circle-question", label: "Help & Support" },
                                        ].map((mi) => (
                                            <a key={mi.label} href="#" className="flex items-center gap-2.5 px-3 py-2 rounded-lg text-white/50 hover:text-white hover:bg-white/[0.04] text-sm transition-colors">
                                                <i className={`${mi.icon} w-4 text-center text-xs`} />{mi.label}
                                            </a>
                                        ))}
                                    </div>
                                    <div className="border-t border-white/[0.06] p-1.5">
                                        <a href="#" className="flex items-center gap-2.5 px-3 py-2 rounded-lg text-error/60 hover:text-error hover:bg-error/5 text-sm transition-colors">
                                            <i className="fa-regular fa-right-from-bracket w-4 text-center text-xs" />Sign Out
                                        </a>
                                    </div>
                                </div>
                            )}
                        ...
                        {/* CTA */}
                        <a href="#" className="cin-header-cta btn btn-primary btn-sm font-semibold border-0 shadow-lg shadow-primary/15 hidden md:inline-flex">
                            Get Started
                        </a>
                        ...
                        >
                            <i className={`fa-regular ${mobileOpen ? "fa-xmark" : "fa-bars"} text-lg transition-transform duration-200`} />
                        </button>
                    </div>
                </div>
            </div>
            ...
            {mobileOpen && (
                <div className="lg:hidden bg-neutral border-t border-white/[0.06] max-h-[80vh] overflow-y-auto">
                    <div className="p-4 border-b border-white/[0.04]">
                        <div className="relative">
                            <i className="fa-regular fa-magnifying-glass absolute left-3 top-1/2 -translate-y-1/2 text-white/30 text-sm" />
                            <input type="text" placeholder="Search..." className="input input-sm w-full pl-9 bg-white/[0.04] border-white/[0.08] text-white placeholder:text-white/30" />
                        </div>
                    </div>
                    <div className="p-2">
                        {NAV_ITEMS.map((item) => (
                            <div key={item.label}>
                                ...
                                >
                                    <span className="flex items-center gap-3 text-sm font-medium">
                                        <i className={`${item.icon} text-white/30 w-5 text-center`} />{item.label}
                                    </span>
                                    {item.children && (
                                        <i className={`fa-regular fa-chevron-down text-[10px] text-white/30 transition-transform duration-200 ${activeDropdown === item.label ? "rotate-180" : ""}`} />
                                    )}
                                </button>
                                {item.children && activeDropdown === item.label && (
                                    <div className="ml-4 pl-4 border-l border-white/[0.06] mb-2">
                                        {item.children.map((child) => (
                                            <a key={child.label} href="#" className="flex items-center gap-3 px-3 py-2.5 text-white/40 hover:text-white/70 transition-colors">
                                                <i className={`${child.icon} text-xs w-4 text-center`} />
                                                <div>
                    ...
                    <div className="p-4 border-t border-white/[0.06] space-y-3">
                        <div className="flex items-center gap-3 px-2 mb-3">
                            <div className="w-9 h-9 rounded-full bg-primary/20 flex items-center justify-center text-xs font-bold text-primary">MC</div>
                            <div>
                                <p className="text-white/70 text-sm font-medium">Marcus Chen</p>
                                <p className="text-white/30 text-xs">Recruiter</p>
                            </div>
                        </div>
                        <a href="#" className="btn btn-primary btn-sm w-full font-semibold border-0">Get Started</a>
                    </div>
                </div>
            )}
        </header>
    ...
    useGSAP(
        () => {
            ...
            if (window.matchMedia("(prefers-reduced-motion: reduce)").matches) {
                gsap.set(containerRef.current.querySelectorAll("[class*='cin-']"), { opacity: 1, y: 0, x: 0 });
                return;
            }

            const tl = gsap.timeline({ defaults: { ease: "power3.out" }, delay: 0.2 });

            tl.fromTo(".cin-header-logo", { opacity: 0, x: -20 }, { opacity: 1, x: 0, duration: 0.6 });
            tl.fromTo(".cin-nav-item", { opacity: 0, y: -10 }, { opacity: 1, y: 0, duration: 0.4, stagger: 0.06 }, 0.2);
            tl.fromTo(".cin-header-action", { opacity: 0, scale: 0.8 }, { opacity: 1, scale: 1, duration: 0.3, stagger: 0.06 }, 0.35);
            tl.fromTo(".cin-header-cta", { opacity: 0, x: 20 }, { opacity: 1, x: 0, duration: 0.5 }, 0.4);
            tl.fromTo(".cin-hero-kicker", { opacity: 0, y: 30 }, { opacity: 1, y: 0, duration: 0.6 }, 0.5);
            tl.fromTo(".cin-hero-headline", { opacity: 0, y: 60 }, { opacity: 1, y: 0, duration: 1 }, 0.6);
            tl.fromTo(".cin-hero-sub", { opacity: 0, y: 20 }, { opacity: 1, y: 0, duration: 0.7 }, 0.9);
            tl.fromTo(".cin-hero-cta-btn", { opacity: 0, y: 15 }, { opacity: 1, y: 0, duration: 0.5, stagger: 0.1 }, 1.1);

            gsap.fromTo(
                ".cin-feature-card",
                { opacity: 0, y: 40 },
                { opacity: 1, y: 0, duration: 0.6, stagger: 0.08, ease: "power2.out", scrollTrigger: { trigger: ".cin-features-grid", start: "top 80%" } },
            );

            gsap.to(".cin-hero-bg", {
                yPercent: 30,
                ease: "none",
                scrollTrigger: { trigger: ".cin-hero-section", start: "top top", end: "bottom top", scrub: true },
            });
        },
        { scope: containerRef },
    );
        ...
        <div ref={containerRef} className="min-h-screen bg-neutral">
            ...
            <section className="cin-hero-section relative min-h-screen flex items-center justify-center overflow-hidden">
                ...
                <div className="relative z-10 max-w-4xl mx-auto px-6 text-center pt-20">
                    ...
                    <p className="cin-hero-sub text-lg md:text-xl text-white/50 max-w-2xl mx-auto leading-relaxed mb-10 opacity-0">
                        The header transitions from transparent to frosted dark glass as you
                        scroll. Hover navigation items for mega-menu dropdowns. Try search,
                        notifications, and the user menu.
                    </p>
                    <div className="flex flex-col sm:flex-row gap-4 justify-center">
                        <a href="#features" className="cin-hero-cta-btn btn btn-primary btn-lg font-semibold border-0 shadow-xl shadow-primary/20 opacity-0">
                            <i className="fa-duotone fa-regular fa-arrow-down" />
                            Explore Features
                        </a>
                        <a href="#" className="cin-hero-cta-btn btn btn-lg font-semibold btn-outline border-white/20 text-white hover:bg-white/10 hover:border-white/40 opacity-0">
            ...
            <section id="features" className="py-24 px-6 border-t border-white/[0.06]">
                <div className="max-w-6xl mx-auto">
                    <div className="text-center mb-16">
                        <p className="text-[10px] uppercase tracking-[0.3em] text-primary/60 font-medium mb-4">Platform Features</p>
                        <h2 className="text-3xl md:text-4xl font-black text-white tracking-tight mb-4">
                            Everything you need to <span className="text-primary">recruit smarter</span>
                        </h2>
                        <p className="text-white/40 max-w-xl mx-auto leading-relaxed">
                            A complete toolkit designed for modern recruiting professionals who demand transparency and results.
                        </p>
                    </div>

                    <div className="cin-features-grid grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {FEATURE_CARDS.map((card, i) => (
                            <div key={i} className="cin-feature-card opacity-0 group p-6 rounded-xl bg-white/[0.02] border border-white/[0.06] hover:border-white/[0.12] hover:bg-white/[0.04] transition-all duration-300">
            ...
            <section className="py-20 px-6 border-t border-white/[0.06] bg-white/[0.01]">
                <div className="max-w-3xl mx-auto text-center">
                    <p className="text-[10px] uppercase tracking-[0.3em] text-white/20 font-medium mb-4">Header Showcase</p>
                    ...
                    <p className="text-white/25 max-w-lg mx-auto leading-relaxed mb-8">
                        The header remains fixed at the top, gaining a frosted backdrop blur and bottom border when the page is scrolled. Navigation dropdowns, search overlay, and user menu are all fully interactive.
                    </p>
                    <div className="flex flex-wrap items-center justify-center gap-6 text-sm">
                        {["Transparent on hero", "Frosted on scroll", "Mega menu dropdowns", "Search overlay", "User menu"].map((feat) => (
                            <div key={feat} className="flex items-center gap-2 text-white/20">
            ...
            <section className="py-24 px-6 border-t border-white/[0.06]">
                <div className="max-w-6xl mx-auto">
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                        {[
                            { value: "10,000+", label: "Active Job Listings", icon: "fa-duotone fa-regular fa-briefcase" },
                            { value: "2,000+", label: "Network Recruiters", icon: "fa-duotone fa-regular fa-users" },
                            { value: "95%", label: "Response Rate", icon: "fa-duotone fa-regular fa-chart-line" },
                        ].map((stat, i) => (
                            <div key={i} className="text-center p-8 rounded-xl bg-white/[0.02] border border-white/[0.06]">
                                <i className={`${stat.icon} text-primary/40 text-2xl mb-4`} />
