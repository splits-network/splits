...
export default function SearchFourPage() {
    ...
    const activeFilters: ActiveFilter[] = [
        ...
        ...(featuredOnly
            ? [{ key: "featured", value: "true", label: "Featured Only" }]
            : []),
        ...(openOnly
            ? [{ key: "open", value: "true", label: "Open Only" }]
            : []),
        ...(salaryMin > 0
            ? [
                  {
                      ...
                      label: `Min Salary: $${(salaryMin / 1000).toFixed(0)}k`,
                  },
              ]
            : []),
        ...(salaryMax < 350000
            ? [
                  {
                      ...
                      label: `Max Salary: $${(salaryMax / 1000).toFixed(0)}k`,
                  },
              ]
            : []),
    ];
    ...
    const removeFilter = (f: ActiveFilter) => {
        if (f.key === "type")
            setSelectedTypes((p) => p.filter((v) => v !== f.value));
        else if (f.key === "level")
            setSelectedLevels((p) => p.filter((v) => v !== f.value));
        else if (f.key === "dept")
            setSelectedDepartments((p) => p.filter((v) => v !== f.value));
        else if (f.key === "loc")
            setSelectedLocations((p) => p.filter((v) => v !== f.value));
    ...
    let results = mockJobs.filter((job) => {
        ...
        const matchesSalary =
            job.salary.min >= salaryMin && job.salary.max <= salaryMax;
    ...
    useEffect(() => {
        if (containerRef.current) {
            gsap.fromTo(
                containerRef.current.querySelector(".cin-search-hero"),
                { opacity: 0, y: 40 },
                {
                    opacity: 1,
                    y: 0,
        ...
        <div ref={containerRef} className="cin-page min-h-screen bg-base-100">
                ...
                <div className="max-w-7xl mx-auto px-6 py-10 lg:py-14">
                    ...
                    <div className="relative max-w-2xl mt-6">
                        ...
                        <input
                            ...
                            className="w-full bg-white/5 border border-white/10 rounded-xl pl-12 pr-4 py-4 text-base text-white placeholder-white/30 focus:outline-none focus:border-coral"
                        />
                        {query && (
                            <button
                                onClick={() => setQuery("")}
                ...
                <div className="bg-base-200 border-b border-base-content/5">
                    <div className="max-w-7xl mx-auto px-6 py-3">
                        <div className="flex items-center gap-3 flex-wrap">
                            <span className="text-xs uppercase tracking-wider text-base-content/40 font-semibold shrink-0">
                                Active Filters:
                            </span>
            ...
            <div className="max-w-7xl mx-auto px-6 py-8">
                <div className="flex gap-8">
                    {/* ── Filters Sidebar ── */}
                    <aside
                        className={`${filtersOpen ? "w-[280px]" : "w-0 overflow-hidden"} shrink-0 transition-all duration-300 hidden lg:block`}
                    >
                        <div className="sticky top-4 space-y-6">
                            ...
                            {/* Job Type */}
                            <div>
                                <h4 className="text-xs font-bold text-base-content/50 mb-3 uppercase tracking-wider">
                                    Job Type
                                </h4>
                                <div className="space-y-2">
                                    {JOB_TYPES.map((t) => (
                                        ...
                                        >
                                            ...
                                            />
                                            <span className="text-sm text-base-content/60 group-hover:text-base-content transition-colors capitalize">
                                                {t}
                                            </span>
                                            <span className="text-xs text-base-content/20 ml-auto">
                                                {
                                                    mockJobs.filter(
                                                        (j) => j.type === t,
                                                    ).length
                                    ...
                                    {LOCATIONS.map((loc) => (
                                        ...
                                        >
                                            <input
                                                ...
                                                className="checkbox checkbox-primary checkbox-sm"
                                            />
                                            <span className="text-sm text-base-content/60 group-hover:text-base-content transition-colors truncate">
                                                {loc}
                                            </span>
                                        </label>
                                    ))}
                    ...
                    <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center justify-between mb-6">
                            <div className="flex items-center gap-3">
                                {!filtersOpen && (
                                    <button
                                        ...
                                        className="hidden lg:flex btn btn-sm bg-base-200 border-base-content/10 font-medium"
                                    >
                            ...
                            <div className="flex items-center gap-3">
                                <button
                                    onClick={() =>
                                        setSaveSearchOpen(!saveSearchOpen)
                                    }
                                ...
                                <select
                                    ...
                                    className="select select-sm select-bordered bg-base-200 border-base-content/10 focus:border-coral font-medium"
                                >
                                    {SORT_OPTIONS.map((o) => (
                        ...
                        {/* Save Search Modal */}
                        {saveSearchOpen && (
                            <div className="mb-6 border border-coral/20 bg-primary/5 rounded-xl p-5">
                                ...
                                <div className="flex gap-3">
                                    <input
                                        ...
                                        className="input input-bordered input-sm bg-base-200 border-base-content/10 flex-1 focus:border-coral focus:outline-none"
                                    />
                                    ...
                                    <button
                                        ...
                                        className="btn btn-sm bg-base-200 border-base-content/10"
                                    >
                                        Cancel
                                    </button>
                                </div>
                                <p className="text-xs text-base-content/40 mt-2">
                                    You will be notified when new jobs match
                                    this search.
                                </p>
                            </div>
                        )}
                            ...
                            {results.length === 0 ? (
                                <div className="text-center py-20">
                                    ...
                                    <p className="text-base-content/40 max-w-sm mx-auto mb-6">
                                        ...
                                        removing some filters to find what you
                                        are looking for.
                                    </p>
                                    <button
                                        onClick={clearAll}
                                ...
                                <div className="space-y-3">
                                    {results.map((job) => (
                                        <div
                                            key={job.id}
                                            className="cin-result-item group bg-base-100 border border-base-content/5 rounded-xl p-5 hover:border-coral/20 hover:shadow-md transition-all cursor-pointer"
                                        >
                                            <div className="flex items-start gap-4">
                                                ...
                                                <img
                                                    ...
                                                    alt=""
                                                    className="w-11 h-11 rounded-xl object-cover ring-1 ring-base-content/10 shrink-0 hidden sm:block"
                                                />
                                                ...
                                                <div className="flex-1 min-w-0">
                                                    <div className="flex items-center gap-2 mb-1 flex-wrap">
                                                        ...
                                                        {job.featured && (
                                                            <span className="badge badge-xs badge-primary font-semibold">
                                                                <i className="fa-duotone fa-regular fa-star mr-0.5" />
                                                                Featured
                                                            </span>
                                                        )}
                                                        <span className="badge badge-xs bg-base-200 border-0 text-base-content/40 capitalize">
                                                            {job.type}
                                                        </span>
                                                    </div>
                                                    ...
                                                    <div className="flex flex-wrap items-center gap-4 mt-2 text-sm text-base-content/40">
                                                        <span className="flex items-center gap-1.5">
                                                            <i className="fa-duotone fa-regular fa-location-dot text-primary text-xs" />
                                                            {job.location}
                                                        </span>
                                                        ...
                                                        <span className="capitalize flex items-center gap-1.5">
                                                            <i className="fa-duotone fa-regular fa-chart-simple text-primary text-xs" />
                                                            {
                                                                job.experienceLevel
                                                            }
                                                        </span>
