...
import { mockCandidateInvitations } from "@/data/mock-candidate-invitations";
import type { CandidateInvitationListing } from "@/types/candidate-invitation-listing";
import { CandidateInvitationsAnimator } from "./candidate-invitations-animator";
...
function statusColor(status: CandidateInvitationListing["status"]) {
    switch (status) {
...
function typeLabel(type: CandidateInvitationListing["type"]) {
    switch (type) {
        case "platform_join": return "Platform Join";
        case "role_application": return "Role Application";
        case "connect": return "Connect";
        case "exclusive_opportunity": return "Exclusive";
    }
}

function typeIcon(type: CandidateInvitationListing["type"]) {
    switch (type) {
        case "platform_join": return "fa-duotone fa-regular fa-user-plus";
        case "role_application": return "fa-duotone fa-regular fa-briefcase";
        case "connect": return "fa-duotone fa-regular fa-link";
        case "exclusive_opportunity": return "fa-duotone fa-regular fa-gem";
    }
}

function channelIcon(channel: CandidateInvitationListing["channel"]) {
    switch (channel) {
        case "email": return "fa-duotone fa-regular fa-envelope";
        case "sms": return "fa-duotone fa-regular fa-message-sms";
        case "in_app": return "fa-duotone fa-regular fa-bell";
        case "linkedin": return "fa-brands fa-linkedin";
    }
}

function channelLabel(channel: CandidateInvitationListing["channel"]) {
    switch (channel) {
...
function InvitationDetail({ invitation, accent, onClose }: { invitation: CandidateInvitationListing; accent: string; onClose?: () => void }) {
    return (
            ...
            <div className="p-6 border-b-4" style={{ borderColor: accent }}>
                <div className="flex items-start justify-between gap-4">
                    <div className="flex-1">
                        ...
                        <div className="flex items-center gap-3 mb-3">
                            <img src={invitation.candidate.avatar} alt={invitation.candidate.name}
                ...
                <div className="flex flex-wrap gap-2 mt-4">
                    ...
                    <span className="px-3 py-1 text-xs font-bold uppercase tracking-wider border-2"
                        ...
                        <i className={`${typeIcon(invitation.type)} mr-1`}></i>
            ...
            <div className="grid grid-cols-3 border-b-4" style={{ borderColor: accent }}>
                <div className="p-4 text-center" style={{ borderRight: `2px solid ${accent}30` }}>
                    <div className="text-lg font-black" style={{ color: accent }}>
                        {invitation.responseTime ? formatResponseTime(invitation.responseTime) : "—"}
                    </div>
                    <div className="text-[10px] font-bold uppercase tracking-wider" style={{ color: COLORS.dark, opacity: 0.5 }}>Response Time</div>
                </div>
                <div className="p-4 text-center" style={{ borderRight: `2px solid ${accent}30` }}>
                ...
                <div className="p-4 text-center">
                    <div className="text-lg font-black" style={{ color: accent }}>{invitation.tags.length}</div>
                    <div className="text-[10px] font-bold uppercase tracking-wider" style={{ color: COLORS.dark, opacity: 0.5 }}>Skills</div>
                </div>
            </div>
            ...
            <div className="p-6">
                ...
                {invitation.role && (
                    <div className="mb-6">
                        ...
                        <div className="p-4 border-2" style={{ borderColor: `${COLORS.teal}60` }}>
                            <div className="font-bold text-sm mb-1" style={{ color: COLORS.dark }}>
                                {invitation.role.title}
                            </div>
                            <div className="text-sm font-semibold mb-1" style={{ color: COLORS.teal }}>
                                {invitation.role.company}
                            </div>
                            <div className="text-xs flex items-center gap-1" style={{ color: COLORS.dark, opacity: 0.6 }}>
                                <i className="fa-duotone fa-regular fa-location-dot"></i>
                                {invitation.role.location}
                            </div>
                        </div>
                    </div>
                )}
                    ...
                    <div className="space-y-3">
                        <div className="flex items-center gap-3">
                            ...
                            <div className="flex-1 flex items-center justify-between">
                                <span className="text-xs font-bold uppercase tracking-wider" style={{ color: COLORS.dark }}>Sent</span>
                                <span className="text-xs" style={{ color: COLORS.dark, opacity: 0.6 }}>{formatDate(invitation.sentDate)}</span>
                            </div>
                        </div>
                        {invitation.respondedDate && (
                            <div className="flex items-center gap-3">
                                <div className="w-2 h-2 rounded-full flex-shrink-0" style={{ backgroundColor: statusColor(invitation.status) }} />
                                <div className="flex-1 flex items-center justify-between">
                                    <span className="text-xs font-bold uppercase tracking-wider" style={{ color: COLORS.dark }}>
                                        {invitation.status === "accepted" ? "Accepted" : invitation.status === "declined" ? "Declined" : "Responded"}
                                    </span>
                                    <span className="text-xs" style={{ color: COLORS.dark, opacity: 0.6 }}>{formatDate(invitation.respondedDate)}</span>
                                </div>
                            </div>
                        )}
                        <div className="flex items-center gap-3">
                            <div className="w-2 h-2 rounded-full flex-shrink-0" style={{ backgroundColor: COLORS.purple }} />
                            <div className="flex-1 flex items-center justify-between">
                                <span className="text-xs font-bold uppercase tracking-wider" style={{ color: COLORS.dark }}>Expires</span>
                                <span className="text-xs" style={{ color: COLORS.dark, opacity: 0.6 }}>{formatDate(invitation.expiryDate)}</span>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                {/* Follow-up count */}
                {invitation.followUpCount > 0 && (
                    <div className="mt-4 p-3 border-2 flex items-center gap-2"
                        style={{ borderColor: `${COLORS.coral}60` }}>
                        <i className="fa-duotone fa-regular fa-paper-plane text-sm" style={{ color: COLORS.coral }}></i>
                        <span className="text-xs font-bold uppercase tracking-wider" style={{ color: COLORS.dark }}>
                            {invitation.followUpCount} Follow-up{invitation.followUpCount > 1 ? "s" : ""} Sent
                        </span>
                    </div>
                )}
                ...
                <div className="mt-4 p-3 border-2 flex items-center gap-2"
                    ...
                    <i className={`${channelIcon(invitation.channel)} text-sm`} style={{ color: COLORS.teal }}></i>
                    <span className="text-xs font-bold uppercase tracking-wider" style={{ color: COLORS.dark }}>
                        Sent via {channelLabel(invitation.channel)}
                    </span>
                </div>
            </div>
        </div>
...
function TableView({ invitations, onSelect, selectedId }: { invitations: CandidateInvitationListing[]; onSelect: (inv: CandidateInvitationListing) => void; selectedId: string | null }) {
    const columnHeaders = ["", "Candidate", "Type", "Role / Company", "Status", "Channel", "Sent", "Response Time"];
                                ...
                                <tr
                                    ...
                                    <td className="px-4 py-3">
                                        <div className="flex items-center gap-2">
                                            ...
                                            <img src={inv.candidate.avatar} alt={inv.candidate.name}
                                                className="w-7 h-7 object-cover border-2" style={{ borderColor: accent }} />
                                            <div>
                                                <span className="font-bold text-sm" style={{ color: COLORS.dark }}>{inv.candidate.name}</span>
                                                {inv.candidate.title && (
                                                    <div className="text-[10px]" style={{ color: COLORS.dark, opacity: 0.5 }}>{inv.candidate.title}</div>
                                                )}
                                            </div>
                                        </div>
                                    </td>
                                    <td className="px-4 py-3">
                                        <span className="text-xs font-bold" style={{ color: accent }}>
                                            <i className={`${typeIcon(inv.type)} mr-1`}></i>
                                            {typeLabel(inv.type)}
                                        </span>
                                    </td>
                                    <td className="px-4 py-3 text-xs" style={{ color: COLORS.dark, opacity: 0.7 }}>
                                        {inv.role ? (
                                            <div>
                                                <div className="font-semibold">{inv.role.title}</div>
                                                <div style={{ opacity: 0.7 }}>{inv.role.company}</div>
                                            </div>
                                        ) : (
                                    ...
                                    <td className="px-4 py-3">
                                        <span className="text-xs" style={{ color: COLORS.dark, opacity: 0.7 }}>
                                    ...
                                    <td className="px-4 py-3 text-xs font-bold" style={{ color: COLORS.dark }}>{formatResponseTime(inv.responseTime)}</td>
                                </tr>
                                {/* Expanded detail row */}
                                {isSelected && (
                                    <tr>
...
function GridView({ invitations, onSelect, selectedId }: { invitations: CandidateInvitationListing[]; onSelect: (inv: CandidateInvitationListing) => void; selectedId: string | null }) {
                ...
                {invitations.map((inv, idx) => {
                    ...
                    return (
                        <div key={inv.id}
                            ...
                            <div className="flex items-center gap-3 mb-3">
                                <img src={inv.candidate.avatar} alt={inv.candidate.name}
                                    className="w-10 h-10 object-cover border-2" style={{ borderColor: accent }} />
                                <div>
                                    ...
                                    {inv.candidate.title && (
                                        <div className="text-[10px] font-semibold" style={{ color: accent }}>{inv.candidate.title}</div>
                                    )}
                                </div>
                            </div>
                            ...
                            {/* Role (if any) */}
                            {inv.role && (
                                <div className="text-xs mb-2" style={{ color: COLORS.dark, opacity: 0.6 }}>
                                    <i className="fa-duotone fa-regular fa-briefcase mr-1"></i>
                                    {inv.role.title} at {inv.role.company}
                                </div>
                            )}
                            ...
                            {/* Channel badge */}
                            <div className="flex items-center gap-1 text-[10px] mb-3" style={{ color: COLORS.dark, opacity: 0.5 }}>
                                <i className={channelIcon(inv.channel)}></i>
                                {channelLabel(inv.channel)}
                            </div>
...
function GmailView({ invitations, onSelect, selectedId }: { invitations: CandidateInvitationListing[]; onSelect: (inv: CandidateInvitationListing) => void; selectedId: string | null }) {
    const selectedInv = invitations.find(inv => inv.id === selectedId);
                        ...
                        <div key={inv.id}
                            ...
                            <div className="flex items-start justify-between gap-2 mb-1">
                                <div className="flex items-center gap-2 min-w-0">
                                    ...
                                    <img src={inv.candidate.avatar} alt={inv.candidate.name}
                                        className="w-6 h-6 object-cover border flex-shrink-0" style={{ borderColor: accent }} />
                            ...
                            {inv.role && (
                                <div className="text-[11px] mb-1" style={{ color: COLORS.dark, opacity: 0.5 }}>
                                    {inv.role.title} at {inv.role.company}
                                </div>
                            )}
...
export default function CandidateInvitationsPage() {
    ...
    const filteredInvitations = useMemo(() => {
        return mockCandidateInvitations.filter(inv => {
            if (searchQuery) {
                const q = searchQuery.toLowerCase();
                const match = inv.candidate.name.toLowerCase().includes(q)
                    || inv.candidate.email.toLowerCase().includes(q)
                    || (inv.candidate.title && inv.candidate.title.toLowerCase().includes(q))
                    || inv.recruiter.name.toLowerCase().includes(q)
                    || inv.recruiter.agency.toLowerCase().includes(q)
                    || (inv.role && inv.role.title.toLowerCase().includes(q))
                    || (inv.role && inv.role.company.toLowerCase().includes(q))
    ...
    const handleSelect = (inv: CandidateInvitationListing) => {
        setSelectedInvId(prev => prev === inv.id ? null : inv.id);
    };

    const stats = {
        total: mockCandidateInvitations.length,
        pending: mockCandidateInvitations.filter(inv => inv.status === "pending").length,
        accepted: mockCandidateInvitations.filter(inv => inv.status === "accepted").length,
        responseRate: Math.round(
            (mockCandidateInvitations.filter(inv => inv.respondedDate).length / mockCandidateInvitations.length) * 100
        ),
    };

    return (
        <CandidateInvitationsAnimator>
            {/* ══════════════════════════════════════════════════════════════
                ...
                <div className="container mx-auto px-4 relative z-10">
                    <div className="max-w-5xl mx-auto text-center">
                        ...
                        <p className="header-subtitle text-lg md:text-xl mb-10 max-w-2xl mx-auto leading-relaxed opacity-0"
                            ...
                            Track recruiter invitations to candidates.
                            Monitor response rates, follow-ups, and engagement.
                        </p>
                        ...
                        <div className="flex flex-wrap justify-center gap-3 mb-8">
                            {[
                                ...
                                { label: "Response Rate", value: `${stats.responseRate}%`, color: COLORS.purple },
                            ].map((stat, i) => (
                        ...
                        <div className="px-4 lg:px-8">
                            ...
                            <div className="controls-bar flex flex-col md:flex-row items-stretch md:items-center gap-4 mb-8 p-4 border-4 opacity-0"
                                ...
                                <div className="flex-1 flex items-center gap-2 px-3 py-2 border-2"
                                    ...
                                    <input
                                        ...
                                        placeholder="Search candidates, recruiters, roles..."
                                ...
                                <div className="flex items-center gap-2">
                                    ...
                                    <select
                                        ...
                                        <option value="role_application">Role Application</option>
                                        <option value="connect">Connect</option>
                                        <option value="exclusive_opportunity">Exclusive</option>
                                    </select>
                                </div>
                            ...
                            <div className="flex items-center justify-between mb-4">
                                <span className="text-xs font-bold uppercase tracking-wider" style={{ color: COLORS.dark, opacity: 0.5 }}>
                                    Showing {filteredInvitations.length} of {mockCandidateInvitations.length} invitations
                                </span>
                                {searchQuery && (
