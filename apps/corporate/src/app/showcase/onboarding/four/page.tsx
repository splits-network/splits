...
const NOTIFICATION_OPTIONS = [
  { key: "new_roles", label: "New matching roles", description: "Get notified when roles match your criteria" },
  { key: "candidate_updates", label: "Candidate updates", description: "Status changes on submitted candidates" },
  { key: "messages", label: "Direct messages", description: "When someone sends you a message" },
  { key: "weekly_digest", label: "Weekly digest", description: "Summary of marketplace activity" },
  { key: "placement_alerts", label: "Placement alerts", description: "When placements are confirmed" },
];
...
export default function OnboardingFourPage() {
    ...
    <div className="cin-step-content flex flex-col items-center text-center max-w-2xl mx-auto gap-8">
      ...
      <div>
        ...
        <p className="text-base-content/60 text-lg leading-relaxed">
          The split-fee recruiting marketplace that connects top recruiters with
          hiring companies. Let&apos;s get your account set up in just a few
          minutes.
        </p>
      </div>
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 w-full mt-4">
        {[
          { icon: "fa-handshake", title: "Split Fees", desc: "Collaborate with other recruiters" },
          { icon: "fa-magnifying-glass-dollar", title: "Find Roles", desc: "Access exclusive job listings" },
          { icon: "fa-chart-line", title: "Grow", desc: "Track your placements & revenue" },
        ].map((item) => (
          <div
            key={item.title}
            className="bg-base-200/50 border border-base-300 rounded-lg p-5 text-center"
          >
            <i className={`fa-duotone fa-regular ${item.icon} text-2xl text-primary mb-3 block`} />
  ...
  const renderProfile = () => (
    <div className="cin-step-content max-w-2xl mx-auto space-y-6">
      <div>
        <h2 className="text-2xl md:text-3xl font-black mb-2">Your Profile</h2>
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-xs text-base-content/40 mt-1">JPG, PNG. Max 2MB.</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <fieldset className="fieldset">
          ...
          <input
            ...
            placeholder="Sarah"
            className="input input-bordered w-full bg-base-200/50 border-base-300 focus:border-primary"
          />
        </fieldset>
        <fieldset className="fieldset">
          <legend className="fieldset-legend text-xs uppercase tracking-wider text-base-content/50">
      ...
      <fieldset className="fieldset">
        ...
        <textarea
          ...
          rows={4}
          className="textarea textarea-bordered w-full bg-base-200/50 border-base-300 focus:border-primary"
        />
  ...
  const renderCompany = () => (
    <div className="cin-step-content max-w-2xl mx-auto space-y-6">
      <div>
        <h2 className="text-2xl md:text-3xl font-black mb-2">Your Company</h2>
        <p className="text-base-content/60">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <fieldset className="fieldset">
          ...
          <select
            ...
            className="select select-bordered w-full bg-base-200/50 border-base-300 focus:border-primary"
          >
      ...
      {/* Logo upload zone */}
      <div className="border-2 border-dashed border-base-300 rounded-lg p-8 text-center hover:border-primary/40 transition-colors cursor-pointer">
  ...
  const renderPreferences = () => (
    <div className="cin-step-content max-w-2xl mx-auto space-y-6">
      <div>
        <h2 className="text-2xl md:text-3xl font-black mb-2">Preferences</h2>
        <p className="text-base-content/60">
      ...
      <div>
        ...
        <div className="space-y-3">
          {NOTIFICATION_OPTIONS.map((opt) => (
            <label
              key={opt.key}
              className="flex items-center justify-between p-3 bg-base-200/50 border border-base-300 rounded-lg cursor-pointer hover:border-primary/30 transition-colors"
            >
              <div>
          ...
          {(["system", "light", "dark"] as const).map((t) => (
            <button
              ...
              className={`btn btn-sm flex-1 capitalize ${
                theme === t ? "btn-primary" : "btn-ghost border border-base-300"
              }`}
            >
              <i
                className={`fa-duotone fa-regular ${
      ...
      {completed ? (
        <>
          ...
          <div>
            ...
            <p className="text-base-content/60 text-lg leading-relaxed">
              Your account is ready. Start exploring the marketplace, find
              matching roles, and connect with other recruiters.
            </p>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 w-full">
            {[
              { icon: "fa-briefcase", label: "Browse Roles", desc: "Explore open positions" },
              { icon: "fa-users", label: "Find Partners", desc: "Connect with recruiters" },
              { icon: "fa-user-plus", label: "Add Candidate", desc: "Submit your first candidate" },
            ].map((action) => (
              <button
                key={action.label}
                className="btn btn-ghost border border-base-300 h-auto py-6 flex-col gap-2 hover:border-primary/40"
              >
                <i className={`fa-duotone fa-regular ${action.icon} text-2xl text-primary`} />
                <span className="font-bold text-sm">{action.label}</span>
                <span className="text-xs text-base-content/50">{action.desc}</span>
              </button>
            ))}
          </div>
        </>
        ...
        <>
          ...
          <div>
            ...
            <p className="text-base-content/60 text-lg leading-relaxed">
              Here&apos;s a summary of your setup. Click finish to complete your
              onboarding.
            </p>
          </div>
          ...
          <div className="w-full bg-base-200/50 border border-base-300 rounded-lg p-6 text-left space-y-4">
            <div className="flex justify-between items-center pb-3 border-b border-base-300">
              <span className="text-sm text-base-content/50">Name</span>
            ...
            <div className="flex justify-between items-center pb-3 border-b border-base-300">
              <span className="text-sm text-base-content/50">Company</span>
            ...
            {selectedIndustries.length > 0 && (
              <div className="flex justify-between items-start pb-3 border-b border-base-300">
                <span className="text-sm text-base-content/50">Industries</span>
                <div className="flex flex-wrap gap-1 justify-end max-w-[60%]">
                  {selectedIndustries.map((ind) => (
                    <span key={ind} className="badge badge-sm badge-primary badge-outline">
                      {ind}
                    </span>
                  ))}
                </div>
            ...
            <div className="flex justify-between items-start">
              <span className="text-sm text-base-content/50">Specializations</span>
              <div className="flex flex-wrap gap-1 justify-end max-w-[60%]">
                {selectedSpecs.map((spec) => (
                  <span key={spec} className="badge badge-sm badge-primary">
                    {spec}
                  </span>
                ))}
              </div>
  ...
  return (
    <div ref={containerRef} className="min-h-screen bg-base-100 flex flex-col">
      {/* Header */}
      <header className="cin-onboard-hero bg-neutral text-neutral-content">
        <div className="max-w-5xl mx-auto px-6 py-6 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <span className="font-black text-lg tracking-tight">Splits</span>
          </div>
          <div className="flex items-center gap-4">
