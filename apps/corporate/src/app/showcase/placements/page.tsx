...
import { mockPlacements } from "@/data/mock-placements";
...
function formatCurrencyFull(amount: number, currency: string) {
    return `${currency} ${amount.toLocaleString()}`;
}

function statusColor(status: PlacementListing["status"]) {
    switch (status) {
        case "active": return COLORS.teal;
        case "completed": return COLORS.purple;
        case "guarantee_period": return COLORS.yellow;
        case "cancelled": return COLORS.coral;
        case "pending_start": return COLORS.yellow;
    }
}

function statusLabel(status: PlacementListing["status"]) {
    switch (status) {
        case "active": return "Active";
        case "completed": return "Completed";
        case "guarantee_period": return "Guarantee";
        case "cancelled": return "Cancelled";
        case "pending_start": return "Pending Start";
    }
}

function invoiceColor(status: PlacementListing["invoiceStatus"]) {
    switch (status) {
        case "paid": return COLORS.teal;
        case "sent": return COLORS.purple;
...
function guaranteeDaysRemaining(expiryStr: string): number {
    const diff = new Date(expiryStr).getTime() - Date.now();
    return Math.max(0, Math.ceil(diff / 86400000));
}

function guaranteeProgress(placement: PlacementListing): number {
    const totalDays = placement.guaranteePeriod;
    const remaining = guaranteeDaysRemaining(placement.guaranteeExpiry);
...
function PlacementDetail({ placement, accent, onClose }: { placement: PlacementListing; accent: string; onClose?: () => void }) {
    const daysRemaining = guaranteeDaysRemaining(placement.guaranteeExpiry);
    const progress = guaranteeProgress(placement);
            ...
            <div className="p-6 border-b-4" style={{ borderColor: accent }}>
                <div className="flex items-start justify-between gap-4">
                    <div className="flex-1">
                        ...
                        <div className="flex flex-wrap items-center gap-3 text-sm">
                            <span className="font-bold" style={{ color: accent }}>{placement.company}</span>
                ...
                <div className="flex flex-wrap gap-2 mt-4">
                    <span className="px-3 py-1 text-xs font-bold uppercase tracking-wider border-2"
                        style={{ borderColor: statusColor(placement.status), color: statusColor(placement.status) }}>
                        {statusLabel(placement.status)}
                    </span>
                    <span className="px-3 py-1 text-xs font-bold uppercase tracking-wider border-2"
                        style={{ borderColor: invoiceColor(placement.invoiceStatus), color: invoiceColor(placement.invoiceStatus) }}>
                        <i className="fa-duotone fa-regular fa-file-invoice mr-1"></i>
            ...
            <div className="grid grid-cols-3 border-b-4" style={{ borderColor: accent }}>
                <div className="p-4 text-center" style={{ borderRight: `2px solid ${accent}30` }}>
                    <div className="text-lg font-black" style={{ color: accent }}>
                        {formatCurrencyFull(placement.salary.amount, placement.salary.currency)}
                    </div>
                    <div className="text-[10px] font-bold uppercase tracking-wider" style={{ color: COLORS.dark, opacity: 0.5 }}>Salary</div>
                </div>
                <div className="p-4 text-center" style={{ borderRight: `2px solid ${accent}30` }}>
                    <div className="text-lg font-black" style={{ color: accent }}>
                        {formatCurrencyFull(placement.fee.amount, placement.fee.currency)}
                    </div>
                    <div className="text-[10px] font-bold uppercase tracking-wider" style={{ color: COLORS.dark, opacity: 0.5 }}>Fee Amount</div>
                </div>
                <div className="p-4 text-center">
                    <div className="text-lg font-black" style={{ color: accent }}>{placement.fee.percentage}%</div>
                    <div className="text-[10px] font-bold uppercase tracking-wider" style={{ color: COLORS.dark, opacity: 0.5 }}>Fee Rate</div>
                </div>
            </div>
            ...
            <div className="p-6">
                ...
                <div className="mb-6">
                    ...
                    <div className="p-4 border-4" style={{ borderColor: COLORS.teal }}>
                        <div className="flex items-center gap-3">
                            <img src={placement.candidate.avatar} alt={placement.candidate.name}
                                className="w-12 h-12 object-cover border-2" style={{ borderColor: COLORS.teal }} />
                            <div>
                                <div className="font-bold text-sm" style={{ color: COLORS.dark }}>{placement.candidate.name}</div>
                                <div className="text-xs" style={{ color: COLORS.teal }}>{placement.candidate.title}</div>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div className="mb-6">
                    ...
                    <div className="p-4 border-4" style={{ borderColor: COLORS.coral }}>
                        <div className="flex items-center gap-3">
                            <img src={placement.recruiter.avatar} alt={placement.recruiter.name}
                                className="w-12 h-12 object-cover border-2" style={{ borderColor: COLORS.coral }} />
                            <div>
                                <div className="font-bold text-sm" style={{ color: COLORS.dark }}>{placement.recruiter.name}</div>
                                <div className="text-xs" style={{ color: COLORS.coral }}>{placement.recruiter.agency}</div>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                {placement.splitPartner && (
                    <div className="mb-6">
                        <h3 className="font-black text-sm uppercase tracking-wider mb-3 flex items-center gap-2"
                            ...
                            Split Partner
                            {placement.splitPercentage && (
                                <span className="text-[10px] font-bold ml-1" style={{ color: COLORS.purple }}>
                                    ({placement.splitPercentage}/{100 - placement.splitPercentage} split)
                                </span>
                            )}
                        </h3>
                        <div className="p-4 border-4" style={{ borderColor: COLORS.purple }}>
                            <div className="flex items-center gap-3">
                                <img src={placement.splitPartner.avatar} alt={placement.splitPartner.name}
                                    className="w-12 h-12 object-cover border-2" style={{ borderColor: COLORS.purple }} />
                                <div>
                                    <div className="font-bold text-sm" style={{ color: COLORS.dark }}>{placement.splitPartner.name}</div>
                                    <div className="text-xs" style={{ color: COLORS.purple }}>{placement.splitPartner.agency}</div>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div className="mb-6">
                    ...
                    <div className="p-4 border-2" style={{ borderColor: `${COLORS.yellow}80` }}>
                        <div className="flex items-center justify-between mb-2">
                            <span className="text-xs font-bold uppercase tracking-wider" style={{ color: COLORS.dark }}>
                                {placement.guaranteePeriod} days
                            </span>
                            <span className="text-xs font-bold" style={{ color: daysRemaining > 0 ? COLORS.teal : COLORS.purple }}>
                                {daysRemaining > 0 ? `${daysRemaining} days remaining` : "Completed"}
                            </span>
                        </div>
                        <div className="w-full h-3 border-2" style={{ borderColor: COLORS.dark, backgroundColor: `${COLORS.dark}10` }}>
                            <div className="h-full transition-all duration-300"
                                style={{
                                    width: `${progress}%`,
                                    backgroundColor: progress >= 100 ? COLORS.purple : COLORS.teal,
                                }} />
                        </div>
                        <div className="flex items-center justify-between mt-2">
                            ...
                            <span className="text-[10px]" style={{ color: COLORS.dark, opacity: 0.5 }}>
                                {new Date(placement.guaranteeExpiry).toLocaleDateString("en-US", { month: "short", day: "numeric" })}
                            </span>
                        </div>
                    </div>
                </div>
                ...
                <div className="mb-6">
                    <div className="p-3 border-2 flex items-center gap-2"
                        style={{ borderColor: `${invoiceColor(placement.invoiceStatus)}60` }}>
                        <i className="fa-duotone fa-regular fa-file-invoice text-sm" style={{ color: invoiceColor(placement.invoiceStatus) }}></i>
                        <span className="text-xs font-bold uppercase tracking-wider" style={{ color: COLORS.dark }}>
                            Invoice: {placement.invoiceStatus}
                        </span>
                        <span className="ml-auto text-xs font-bold" style={{ color: invoiceColor(placement.invoiceStatus) }}>
                            {formatCurrencyFull(placement.fee.amount, placement.fee.currency)}
                        </span>
                    </div>
                </div>
                ...
                <div className="mb-6">
                    <div className="p-3 border-2 flex items-center gap-2"
                        ...
                        <i className="fa-duotone fa-regular fa-calendar-check text-sm" style={{ color: COLORS.teal }}></i>
                        <span className="text-xs font-bold uppercase tracking-wider" style={{ color: COLORS.dark }}>
                            Start Date: {new Date(placement.startDate).toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" })}
                        </span>
                    </div>
                </div>
                ...
                <div className="p-3 border-2 flex items-center gap-2"
                    ...
                    <span className="text-xs font-bold uppercase tracking-wider" style={{ color: COLORS.dark }}>
                        Placed: {new Date(placement.placementDate).toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" })}
                    </span>
                </div>
            </div>
        </div>
...
function TableView({ placements, onSelect, selectedId }: { placements: PlacementListing[]; onSelect: (p: PlacementListing) => void; selectedId: string | null }) {
    const columnHeaders = ["", "Role", "Company", "Candidate", "Salary", "Fee", "Status", "Start Date"];
                                ...
                                <tr
                                    ...
                                    <td className="px-4 py-3">
                                        <div className="flex items-center gap-2">
                                            ...
                                            <span className="font-bold text-sm" style={{ color: COLORS.dark }}>{placement.jobTitle}</span>
                                        </div>
                                    </td>
                                    <td className="px-4 py-3 text-sm font-semibold" style={{ color: accent }}>{placement.company}</td>
                                    <td className="px-4 py-3">
                                        <div className="flex items-center gap-2">
                                            <img src={placement.candidate.avatar} alt={placement.candidate.name}
                                                className="w-6 h-6 object-cover border-2" style={{ borderColor: accent }} />
                                            <span className="text-xs font-bold" style={{ color: COLORS.dark }}>{placement.candidate.name}</span>
                                        </div>
                                    </td>
                                    <td className="px-4 py-3 text-xs font-bold" style={{ color: COLORS.dark }}>
                                        {formatCurrency(placement.salary.amount, placement.salary.currency)}
                                    </td>
                                    <td className="px-4 py-3 text-xs font-bold" style={{ color: COLORS.dark }}>
                                        {formatCurrency(placement.fee.amount, placement.fee.currency)}
                                    </td>
                                    <td className="px-4 py-3">
                                        <span className="px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider"
                                            style={{
                                                backgroundColor: statusColor(placement.status),
                                                color: statusColor(placement.status) === COLORS.yellow ? COLORS.dark : COLORS.white,
                                            }}>
                                            {statusLabel(placement.status)}
                                        </span>
                                    </td>
                                ...
                                {isSelected && (
                                    <tr>
                                        <td colSpan={colCount} className="p-0"
                                            ...
                                            <PlacementDetail placement={placement} accent={accent} onClose={() => onSelect(placement)} />
                                        </td>
                                    </tr>
                                )}
                            </Fragment>
...
function GridView({ placements, onSelect, selectedId }: { placements: PlacementListing[]; onSelect: (p: PlacementListing) => void; selectedId: string | null }) {
    const selectedPlacement = placements.find(p => p.id === selectedId);

    return (
        <div className="flex gap-6">
            {/* Cards Grid */}
            <div className={`grid gap-4 ${selectedPlacement ? "w-1/2 grid-cols-1 lg:grid-cols-2" : "w-full grid-cols-1 sm:grid-cols-2 lg:grid-cols-3"}`}>
                {placements.map((placement, idx) => {
                    const accent = ACCENT_CYCLE[idx % 4];
                    const isSelected = selectedId === placement.id;
                    const progress = guaranteeProgress(placement);
                    ...
                    return (
                        <div key={placement.id}
                            ...
                            <div className="text-sm font-bold mb-2" style={{ color: accent }}>{placement.company}</div>
                            ...
                            <div className="flex items-center justify-between mb-2">
                                ...
                                <span className="text-xs font-bold" style={{ color: accent }}>
                                    {formatCurrency(placement.fee.amount, placement.fee.currency)} fee
                                </span>
                            </div>
                            ...
                            <div className="mb-3">
                                <div className="flex items-center justify-between mb-1">
                                    ...
                                    <span className="text-[10px] font-bold" style={{ color: daysRemaining > 0 ? COLORS.teal : COLORS.purple }}>
                                        {daysRemaining > 0 ? `${daysRemaining}d left` : "Done"}
                                    </span>
                                </div>
                                <div className="w-full h-2 border-2" style={{ borderColor: `${COLORS.dark}30`, backgroundColor: `${COLORS.dark}10` }}>
                                    <div className="h-full transition-all duration-300"
                                        style={{
                                            width: `${progress}%`,
                                            backgroundColor: progress >= 100 ? COLORS.purple : COLORS.teal,
                            ...
                            <div className="flex flex-wrap gap-1">
                                {placement.tags.slice(0, 3).map((tag, i) => (
                            ...
                            <div className="flex items-center gap-3 mt-3 pt-3" style={{ borderTop: `2px solid ${accent}30` }}>
                                ...
                                <div>
                                    <div className="text-xs font-bold" style={{ color: COLORS.dark }}>{placement.recruiter.name}</div>
                                    <div className="text-[10px]" style={{ color: COLORS.dark, opacity: 0.5 }}>{placement.recruiter.agency}</div>
                                </div>
                            </div>
                        </div>
                    );
            ...
            {selectedPlacement && (
                <div className="w-1/2 border-4 flex-shrink-0 sticky top-4 self-start"
                    style={{ borderColor: ACCENT_CYCLE[placements.indexOf(selectedPlacement) % 4], backgroundColor: COLORS.white, maxHeight: "calc(100vh - 2rem)" }}>
                    <PlacementDetail placement={selectedPlacement} accent={ACCENT_CYCLE[placements.indexOf(selectedPlacement) % 4]} onClose={() => onSelect(selectedPlacement)} />
                </div>
            )}
        </div>
    );
...
function GmailView({ placements, onSelect, selectedId }: { placements: PlacementListing[]; onSelect: (p: PlacementListing) => void; selectedId: string | null }) {
    const selectedPlacement = placements.find(p => p.id === selectedId);
    ...
    return (
        <div className="flex gap-0 border-4" style={{ borderColor: COLORS.dark, minHeight: 600 }}>
                        ...
                        <div key={placement.id}
                            ...
                            <div className="flex items-start justify-between gap-2 mb-1">
                                ...
                                <span className="text-[10px] font-bold flex-shrink-0 whitespace-nowrap"
                                    style={{ color: COLORS.dark, opacity: 0.4 }}>
                                    {timeAgo(placement.placementDate)}
                                </span>
                            </div>
                            <div className="text-xs font-bold mb-1" style={{ color: accent }}>{placement.company}</div>
                            <div className="flex items-center gap-2 mb-1">
                                <img src={placement.candidate.avatar} alt={placement.candidate.name}
                                    className="w-5 h-5 object-cover border" style={{ borderColor: accent }} />
                                <span className="text-[11px] font-semibold" style={{ color: COLORS.dark, opacity: 0.7 }}>
            ...
            <div className="w-3/5 overflow-hidden" style={{ backgroundColor: COLORS.white }}>
                {selectedPlacement ? (
                    <PlacementDetail placement={selectedPlacement} accent={ACCENT_CYCLE[placements.indexOf(selectedPlacement) % 4]} />
                ) : (
...
export default function PlacementsPage() {
    ...
    const filteredPlacements = useMemo(() => {
        return mockPlacements.filter(placement => {
            if (searchQuery) {
                const q = searchQuery.toLowerCase();
                const match = placement.jobTitle.toLowerCase().includes(q)
                    || placement.company.toLowerCase().includes(q)
                    || placement.candidate.name.toLowerCase().includes(q)
                    || placement.recruiter.name.toLowerCase().includes(q)
                    || placement.location.toLowerCase().includes(q)
                    || placement.tags.some(t => t.toLowerCase().includes(q));
                if (!match) return false;
            }
            if (statusFilter !== "all" && placement.status !== statusFilter) return false;
            if (invoiceFilter !== "all" && placement.invoiceStatus !== invoiceFilter) return false;
            return true;
        });
    }, [searchQuery, statusFilter, invoiceFilter]);

    const handleSelect = (placement: PlacementListing) => {
        setSelectedId(prev => prev === placement.id ? null : placement.id);
    };

    const totalRevenue = mockPlacements.reduce((sum, p) => {
        // Normalize to USD for display (simplified)
        return sum + p.fee.amount;
    }, 0);

    const stats = {
        total: mockPlacements.length,
        active: mockPlacements.filter(p => p.status === "active").length,
        guarantee: mockPlacements.filter(p => p.status === "guarantee_period").length,
        revenue: totalRevenue >= 1000 ? `$${Math.round(totalRevenue / 1000)}K` : `$${totalRevenue}`,
    };
                    ...
                    <div className="max-w-5xl mx-auto text-center">
                        ...
                        <div className="flex flex-wrap justify-center gap-3 mb-8">
                            {[
                                ...
                                { label: "In Guarantee", value: stats.guarantee, color: COLORS.yellow },
                                { label: "Revenue", value: stats.revenue, color: COLORS.purple },
                            ].map((stat, i) => (
                        ...
                        <div className="px-4 lg:px-8">
                            ...
                            <div className="controls-bar flex flex-col md:flex-row items-stretch md:items-center gap-4 mb-8 p-4 border-4 opacity-0"
                                ...
                                <div className="flex-1 flex items-center gap-2 px-3 py-2 border-2"
                                    ...
                                    <input
                                        ...
                                        placeholder="Search roles, candidates, companies..."
                                ...
                                <div className="flex items-center gap-2">
                                    ...
                                    <select
                                        ...
                                        <option value="guarantee_period">Guarantee</option>
                                ...
                                {/* Invoice filter */}
                                <div className="flex items-center gap-2">
                                    <span className="text-[10px] font-bold uppercase tracking-wider" style={{ color: COLORS.dark, opacity: 0.5 }}>Invoice:</span>
                                    <select
                                        value={invoiceFilter}
                            ...
                            <div className="flex items-center justify-between mb-4">
                                <span className="text-xs font-bold uppercase tracking-wider" style={{ color: COLORS.dark, opacity: 0.5 }}>
                                    Showing {filteredPlacements.length} of {mockPlacements.length} placements
                                </span>
                                {searchQuery && (
                            ...
                            <div className="listings-content opacity-0">
                                {filteredPlacements.length === 0 ? (
                                    <div className="text-center py-20 border-4" style={{ borderColor: `${COLORS.dark}20`, backgroundColor: COLORS.white }}>
                                        ...
                                        <button onClick={() => { setSearchQuery(""); setStatusFilter("all"); setInvoiceFilter("all"); }}
                                    ...
                                    <>
                                        {viewMode === "table" && (
                                            <TableView placements={filteredPlacements} onSelect={handleSelect} selectedId={selectedId} />
                                        )}
                                        {viewMode === "grid" && (
                                            <GridView placements={filteredPlacements} onSelect={handleSelect} selectedId={selectedId} />
                                        )}
                                        {viewMode === "gmail" && (
                                            <GmailView placements={filteredPlacements} onSelect={handleSelect} selectedId={selectedId} />
                                        )}
                                    </>
                                )}
                            </div>
