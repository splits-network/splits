...
interface CookieAnimatorProps {
    children: ReactNode;
}

export function CookieAnimator({ children }: CookieAnimatorProps) {
    ...
    useGSAP(
        () => {
            ...
            if (shapes.length) {
                gsap.fromTo(shapes, { opacity: 0, scale: 0, rotation: -180 }, {
            ...
            if (toc) gsap.fromTo(toc, { opacity: 0, y: 30 }, { opacity: 1, y: 0, duration: D.normal, ease: E.smooth, scrollTrigger: { trigger: toc, start: "top 85%" } });

            $(".legal-section").forEach((section) => {
                gsap.fromTo(section, { opacity: 0, y: 40 }, { opacity: 1, y: 0, duration: D.slow, ease: E.smooth, scrollTrigger: { trigger: section, start: "top 80%" } });
            });

            const contact = $1(".legal-contact");
            if (contact) gsap.fromTo(contact, { opacity: 0, y: 30 }, { opacity: 1, y: 0, duration: D.normal, ease: E.bounce, scrollTrigger: { trigger: contact, start: "top 85%" } });
        },
        { scope: containerRef },
    );
