...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
    ...
    return {
        title: `${label} | Employment Networks`,
        description: `Browse ${label.toLowerCase()} from Employment Networks.`,
        ...buildCanonical(`/cms/${type}`),
    };
}
...
export default async function CmsTypePage({ params, searchParams }: PageProps) {
    ...
    const [{ data: pages, pagination }, tags] = await Promise.all([
        getContentPagesByType(type as ContentPageType, {
            tag,
            page: currentPage,
            limit: 12,
        }),
    ...
    return (
        <div className="max-w-5xl mx-auto px-6 py-12">
            ...
            <p className="text-base-content/60 mb-8">
                Browse {label.toLowerCase()} from Employment Networks.
            </p>
            ...
            {tags.length > 0 && (
                <div className="flex flex-wrap gap-2 mb-8">
                    <Link
                        href={`/cms/${type}`}
                        className={`badge badge-lg ${!tag ? 'badge-primary' : 'badge-ghost'}`}
                    >
                        All
                    </Link>
                    {tags.map((t) => (
                        <Link
                            key={t.id}
                            href={`/cms/${type}?tag=${t.slug}`}
                            className={`badge badge-lg ${tag === t.slug ? 'badge-primary' : 'badge-ghost'}`}
                        >
                            {t.name}
                        </Link>
                    ))}
                        ...
                        >
                            <div className="card-body">
                                ...
                                <div className="flex flex-wrap gap-1 mt-2">
                                    {page.tags?.map((t) => (
                                        <span key={t.id} className="badge badge-sm badge-ghost">
                                            {t.name}
                                        </span>
                                    ))}
                                </div>
                                {page.published_at && (
                                    <p className="text-xs text-base-content/40 mt-2" suppressHydrationWarning>
            ...
            {pagination.total_pages > 1 && (
                <div className="flex justify-center gap-2 mt-8">
                    {Array.from({ length: pagination.total_pages }, (_, i) => i + 1).map((p) => (
                        <Link
                            key={p}
                            href={`/cms/${type}?${new URLSearchParams({
                                ...(tag ? { tag } : {}),
                                page: String(p),
                            })}`}
