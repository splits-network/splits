...
export function OnboardingProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    const fetchOnboardingStatus = async () => {
      ...
      try {
        ...
        try {
          const meResponse = await apiClient.get<{ data: CandidateData }>(
            "/candidates/me",
          );
          if (meResponse?.data) {
        ...
        if (!candidateData) {
          throw new Error("Unable to load or create candidate profile");
        }

        setCandidateId(candidateData.id);

        // Check onboarding status and show modal if needed
        const shouldShowModal = candidateData.onboarding_status === "pending";
        ...
        const profileData: CandidateProfileData = {
          ...
          // Job preferences
          desired_job_type: parseJobTypes(candidateData.desired_job_type),
          availability: candidateData.availability || "",
          open_to_remote: candidateData.open_to_remote || false,
          open_to_relocation: candidateData.open_to_relocation || false,
          desired_salary_min: candidateData.desired_salary_min || undefined,
          desired_salary_max: candidateData.desired_salary_max || undefined,
        };
  ...
  const previousStep = () => {
    setState((prev) => {
      const prev_step = Math.max(prev.currentStep - 1, 1) as OnboardingStep;
  ...
  const skipOnboarding = async () => {
    ...
    try {
      ...
      console.error("[OnboardingProvider] Failed to skip onboarding:", error);
      setState((prev) => ({
        ...prev,
        error:
  ...
  const completeOnboarding = async () => {
    ...
    try {
      ...
      if (profileData.full_name) {
        updatePayload.name = profileData.full_name;
      }
      ...
      if (profileData.open_to_relocation !== undefined) {
        updatePayload.open_to_relocation = profileData.open_to_relocation;
      }
      if (profileData.desired_salary_min !== undefined) {
        updatePayload.desired_salary_min = profileData.desired_salary_min;
      }
      if (profileData.desired_salary_max !== undefined) {
        updatePayload.desired_salary_max = profileData.desired_salary_max;
      }
        ...
        <div className="fixed inset-0 bg-base-300/80 z-[9999] flex items-center justify-center">
          <div className="card bg-base-100 shadow-xl max-w-md w-full mx-4">
            <div className="card-body items-center text-center">
              ...
              <p className="text-sm text-base-content/70">{initMessage}</p>
