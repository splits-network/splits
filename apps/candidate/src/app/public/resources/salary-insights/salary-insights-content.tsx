...
const insights = [
    { role: "Software Engineer", avgSalary: "$125,000", range: "$95K - $170K", growth: "+8%", icon: "code", color: "primary" },
    { role: "Product Manager", avgSalary: "$135,000", range: "$105K - $180K", growth: "+12%", icon: "lightbulb", color: "secondary" },
    { role: "Sales Director", avgSalary: "$145,000", range: "$110K - $200K", growth: "+6%", icon: "handshake", color: "accent" },
    { role: "UX Designer", avgSalary: "$105,000", range: "$80K - $140K", growth: "+10%", icon: "palette", color: "info" },
    { role: "Marketing Manager", avgSalary: "$95,000", range: "$75K - $130K", growth: "+7%", icon: "bullhorn", color: "warning" },
    { role: "Data Analyst", avgSalary: "$90,000", range: "$70K - $120K", growth: "+15%", icon: "chart-simple", color: "success" },
];

const factors = [
    { title: "Location", description: "Geographic location significantly impacts salary. Major tech hubs often pay 20-40% more.", icon: "location-dot" },
    { title: "Experience", description: "Years of experience and expertise level are key drivers of compensation.", icon: "briefcase" },
    { title: "Company Size", description: "Startups vs. enterprises offer different comp structures and equity opportunities.", icon: "building" },
    { title: "Industry", description: "Tech, finance, and healthcare sectors typically offer higher compensation.", icon: "industry" },
];
...
export function SalaryInsightsContent() {
    ...
    useGSAP(
        () => {
            ...
            const heading = salariesRef.current.querySelector(".section-heading");
            const cards = salariesRef.current.querySelectorAll(".salary-card");
            const icons = salariesRef.current.querySelectorAll(".salary-icon");

            const tl = gsap.timeline({ scrollTrigger: { trigger: salariesRef.current, start: "top 80%" } });
    ...
    useGSAP(
        () => {
            ...
            const heading = factorsRef.current.querySelector(".section-heading");
            const cards = factorsRef.current.querySelectorAll(".factor-card");

            const tl = gsap.timeline({ scrollTrigger: { trigger: factorsRef.current, start: "top 80%" } });
            if (heading) tl.fromTo(heading, { opacity: 0, y: 30 }, { opacity: 1, y: 0, duration: duration.normal, ease: easing.smooth });
            if (cards.length > 0) tl.fromTo(cards, { opacity: 0, x: -30 }, { opacity: 1, x: 0, duration: duration.normal, ease: easing.smooth, stagger: stagger.normal }, "-=0.3");
        },
        { scope: factorsRef }
    );
    ...
    return (
        <div className="min-h-screen bg-base-200">
            {/* Header */}
            <div ref={heroRef} className="bg-gradient-to-br from-success to-accent text-white py-16 overflow-hidden">
            ...
            <div className="container mx-auto px-4 py-12">
                <div ref={salariesRef} className="mb-16 overflow-hidden">
                    <h2 className="section-heading text-3xl font-bold mb-8">Average Salaries by Role</h2>
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {insights.map((insight, index) => (
                            <div key={index} className={`salary-card card bg-base-100 shadow border-t-4 border-${insight.color}`}>
                                <div className="card-body">
                                    <div className="flex items-center gap-3 mb-4">
                                        <div className={`salary-icon w-12 h-12 rounded-full bg-${insight.color}/20 flex items-center justify-center`}>
                ...
                <div ref={factorsRef} className="mb-16 overflow-hidden">
                    <h2 className="section-heading text-3xl font-bold mb-8">What Affects Your Salary?</h2>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        {factors.map((factor, index) => (
                            <div key={index} className="factor-card card bg-base-100 shadow">
                                <div className="card-body">
