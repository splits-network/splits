...
export function SuccessStoriesContent() {
    ...
    useGSAP(
        () => {
            ...
            const tl = gsap.timeline({ scrollTrigger: { trigger: statsRef.current, start: "top 85%" } });
            if (cards.length > 0) tl.fromTo(cards, { opacity: 0, y: 30, scale: 0.9 }, { opacity: 1, y: 0, scale: 1, duration: duration.normal, ease: easing.bounce, stagger: stagger.tight });
            if (icons.length > 0) tl.fromTo(icons, { scale: 0 }, { scale: 1, duration: duration.fast, ease: easing.bounce, stagger: stagger.tight }, "-=0.5");
        },
        { scope: statsRef }
    );
    ...
    useGSAP(
        () => {
            ...
            const tl = gsap.timeline({ scrollTrigger: { trigger: storiesRef.current, start: "top 80%" } });
            if (cards.length > 0) tl.fromTo(cards, { opacity: 0, y: 40, scale: 0.95 }, { opacity: 1, y: 0, scale: 1, duration: duration.normal, ease: easing.smooth, stagger: stagger.normal });
        },
        { scope: storiesRef }
    );
    ...
    return (
        <div className="min-h-screen bg-base-200">
            {/* Header */}
            <div ref={heroRef} className="bg-gradient-to-br from-warning to-accent text-white py-16 overflow-hidden">
            ...
            <div className="container mx-auto px-4 py-12">
                <div ref={statsRef} className="grid grid-cols-2 md:grid-cols-4 gap-6 mb-16 overflow-hidden">
                    {stats.map((stat, index) => (
                        <div key={index} className="stat-card card bg-base-100 shadow">
                            <div className="card-body text-center">
                                <i className={`stat-icon fa-duotone fa-regular fa-${stat.icon} text-4xl text-primary mb-2`}></i>
                ...
                {/* Stories Grid */}
                <div ref={storiesRef} className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-16 overflow-hidden">
                    {stories.map((story, index) => (
                        <div key={index} className="story-card card bg-base-100 shadow hover:shadow-lg transition-shadow">
