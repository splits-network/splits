...
const trends = [
    { title: "Remote Work Revolution", description: "Remote and hybrid work models continue to dominate, with 74% of companies offering flexible work arrangements.", icon: "house-laptop", color: "primary", impact: "High", growth: "+45%" },
    { title: "AI & Automation", description: "AI integration across industries is creating new roles while transforming existing ones. Demand for AI skills up 200%.", icon: "robot", color: "secondary", impact: "Very High", growth: "+200%" },
    { title: "Skills-Based Hiring", description: "Companies prioritizing skills over degrees, focusing on practical abilities and project portfolios.", icon: "graduation-cap", color: "accent", impact: "High", growth: "+60%" },
    { title: "Mental Health Focus", description: "Companies investing in employee well-being programs, mental health support, and work-life balance initiatives.", icon: "heart-pulse", color: "success", impact: "Medium", growth: "+35%" },
    { title: "Gig Economy Growth", description: "Rise of contract and freelance work, with 36% of workers participating in the gig economy.", icon: "handshake", color: "warning", impact: "High", growth: "+28%" },
    { title: "Diversity & Inclusion", description: "Increased focus on diverse hiring practices and inclusive workplace cultures across all industries.", icon: "users", color: "info", impact: "High", growth: "+52%" },
];

const sectors = [
    { name: "Technology", growth: "Strong", hotRoles: ["AI Engineer", "Cloud Architect", "Data Scientist"], icon: "laptop-code", color: "primary" },
    { name: "Healthcare", growth: "Very Strong", hotRoles: ["Nurse Practitioner", "Health Informatics", "Telehealth Specialist"], icon: "briefcase-medical", color: "error" },
    { name: "Green Energy", growth: "Explosive", hotRoles: ["Solar Engineer", "Sustainability Consultant", "EV Technician"], icon: "leaf", color: "success" },
    { name: "E-commerce", growth: "Strong", hotRoles: ["UX Designer", "Digital Marketing", "Supply Chain Manager"], icon: "cart-shopping", color: "warning" },
];
...
export function IndustryTrendsContent() {
    ...
    useGSAP(
        () => {
            ...
            const tl = gsap.timeline({ scrollTrigger: { trigger: trendsRef.current, start: "top 80%" } });
            if (heading) tl.fromTo(heading, { opacity: 0, y: 30 }, { opacity: 1, y: 0, duration: duration.normal, ease: easing.smooth });
            if (cards.length > 0) tl.fromTo(cards, { opacity: 0, y: 40, scale: 0.95 }, { opacity: 1, y: 0, scale: 1, duration: duration.normal, ease: easing.smooth, stagger: stagger.tight }, "-=0.3");
    ...
    useGSAP(
        () => {
            ...
            const heading = sectorsRef.current.querySelector(".section-heading");
            const cards = sectorsRef.current.querySelectorAll(".sector-card");

            const tl = gsap.timeline({ scrollTrigger: { trigger: sectorsRef.current, start: "top 80%" } });
            if (heading) tl.fromTo(heading, { opacity: 0, y: 30 }, { opacity: 1, y: 0, duration: duration.normal, ease: easing.smooth });
            if (cards.length > 0) tl.fromTo(cards, { opacity: 0, x: -40 }, { opacity: 1, x: 0, duration: duration.normal, ease: easing.smooth, stagger: stagger.normal }, "-=0.3");
        },
        { scope: sectorsRef }
    );
    ...
    useGSAP(
        () => {
            ...
            const tl = gsap.timeline({ scrollTrigger: { trigger: skillsRef.current, start: "top 80%" } });
            if (heading) tl.fromTo(heading, { opacity: 0, y: 30 }, { opacity: 1, y: 0, duration: duration.normal, ease: easing.smooth });
            if (card) tl.fromTo(card, { opacity: 0, scale: 0.95 }, { opacity: 1, scale: 1, duration: duration.normal, ease: easing.smooth }, "-=0.3");
            if (bars.length > 0) tl.fromTo(bars, { scaleX: 0 }, { scaleX: 1, duration: duration.normal, ease: easing.smooth, stagger: stagger.tight }, "-=0.3");
        },
        { scope: skillsRef }
    );
    ...
    useGSAP(
        () => {
            ...
            const items = takeawaysRef.current.querySelectorAll(".takeaway-item");

            const tl = gsap.timeline({ scrollTrigger: { trigger: takeawaysRef.current, start: "top 80%" } });
            if (card) tl.fromTo(card, { opacity: 0, y: 30, scale: 0.95 }, { opacity: 1, y: 0, scale: 1, duration: duration.normal, ease: easing.smooth });
    ...
    return (
        <div className="min-h-screen bg-base-200">
            {/* Header */}
            <div ref={heroRef} className="bg-gradient-to-br from-accent to-secondary text-white py-16 overflow-hidden">
                <div className="container mx-auto px-4">
                    <div className="max-w-3xl">
                        <div className="flex items-center gap-2 mb-4">
                            <i className="hero-icon fa-duotone fa-regular fa-trending-up text-3xl"></i>
            ...
            <div className="container mx-auto px-4 py-12">
                ...
                <div ref={trendsRef} className="mb-16 overflow-hidden">
                    <h2 className="section-heading text-3xl font-bold mb-8">Major Trends Shaping 2025</h2>
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {trends.map((trend, index) => (
                            <div key={index} className="trend-card card bg-base-100 shadow hover:shadow-lg transition-shadow">
                                <div className="card-body">
                                    <div className={`trend-icon w-14 h-14 rounded-full bg-${trend.color}/20 flex items-center justify-center mb-4`}>
                                    ...
                                    <p className="text-sm text-base-content/70 flex-grow">{trend.description}</p>
                ...
                <div ref={sectorsRef} className="mb-16 overflow-hidden">
                    <h2 className="section-heading text-3xl font-bold mb-8">Fastest Growing Sectors</h2>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        {sectors.map((sector, index) => (
                            <div key={index} className="sector-card card bg-base-100 shadow">
                                <div className="card-body">
                ...
                <div ref={skillsRef} className="mb-16 overflow-hidden">
                    <h2 className="section-heading text-3xl font-bold mb-8">Most In-Demand Skills</h2>
                        ...
                        <div className="card-body">
                            ...
                            <ul className="space-y-3">
                                {takeaways.map((item, index) => (
                                    <li key={index} className="takeaway-item flex items-start gap-3">
