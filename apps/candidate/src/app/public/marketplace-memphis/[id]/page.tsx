...
import RecruiterDetailMemphis from './recruiter-detail-memphis';
...
export async function generateMetadata({
    ...
    try {
        const response = await apiClient.get<{ data: Recruiter }>(`/recruiters/${id}`, {
            params: { include: 'user' }
        });
    ...
    return {
        title: `${name} - ${tagline}`,
        description: recruiter?.bio?.substring(0, 160) || `View ${name}'s recruiter profile`,
        openGraph: {
            title: `${name} - ${tagline}`,
            description: recruiter?.bio?.substring(0, 160) || `View ${name}'s recruiter profile`,
...
export default async function RecruiterDetailPage({
    ...
    return <RecruiterDetailMemphis recruiter={recruiter} />;
}
