...
export default function ApplicationsClient() {
    ...
    const fetchApplications = useCallback(async (params: Record<string, any>) => {
        ...
        const response = await client.get('/applications', { params: { ...params, include: 'job,recruiter,ai_review' } });
                ...
                {viewMode === 'table' && archivedApps.length > 0 && (
                    <div>
                        ...
                        <div className="opacity-70">
                            <DataTable
                                columns={applicationColumns.filter(col => !['recruiter', 'actions'].includes(col.key))}
                                sortBy={sortBy}
                                sortOrder={sortOrder}
                                onSort={handleSort}
