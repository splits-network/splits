...
const SAVED_SEARCHES_KEY = 'splits:saved-searches';
...
function sortResults(results: SearchResult[], sortBy: SortOption): SearchResult[] {
  ...
  switch (sortBy) {
    ...
    case 'oldest':
      return sorted.reverse(); // Backend returns newest first, so reverse
    case 'newest':
    default:
      return sorted; // Backend already sorts by updated_at DESC
  }
}
...
export function useFullSearch(): UseFullSearchReturn {
  ...
  const fetchResults = useCallback(
    async (
      ...
      try {
        ...
        // Strategy: if 0 or 1 entity type selected → single API call with server pagination
        //           if 2+ types → parallel calls per type, merge, compute pagination client-side
        ...
        if (types.length <= 1) {
          ...
          setRawResults(response.data.data);
          ...
          const promises = types.map((type) => {
            const typeParams: Record<string, any> = {
              ...
              mode: 'full',
              page: 1,
              limit: 100, // Fetch max per type, paginate client-side
              entity_type: type,
            };
  ...
  // Execute search immediately (search button / Enter)
  ...
  const setSortBy = useCallback(
    (sort: SortOption) => {
      ...
      // No re-fetch needed — sorting is client-side
    },
