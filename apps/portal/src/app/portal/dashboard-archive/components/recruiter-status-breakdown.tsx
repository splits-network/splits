...
export default function RecruiterStatusBreakdown({
    stats,
    loading,
}: RecruiterStatusBreakdownProps) {
    if (loading) {
        return (
            <MemphisCard
    ...
    const totalRecruiters =
        recruiter_statuses.active +
        recruiter_statuses.pending +
        recruiter_statuses.suspended;
    ...
    const recruiterSegments = [
        ...
        {
            label: "Suspended",
            count: recruiter_statuses.suspended,
            accentIdx: 0,
        },
    ];
    ...
    return (
        ...
        >
            ...
            <div className="mb-5">
                ...
                {totalRecruiters > 0 ? (
                    <div className="h-5 border-3 border-dark flex overflow-hidden">
                    ...
                    <div className="h-5 border-2 border-dark bg-dark/5" />
                )}
                    ...
                    {recruiterSegments.map((seg) => {
                        ...
                        return (
                            ...
                            >
                                ...
                                <span className="font-black tabular-nums text-dark">
                                    {seg.count}
                                </span>
                            </div>
                        );
            ...
            <div className="border-t-2 border-dark pt-4 space-y-2">
                {userRoles.map((role) => {
                    ...
                    return (
                        ...
                        >
                            <div
                                className={`w-7 h-7 border-3 border-dark ${accent.bg}/20 flex items-center justify-center shrink-0`}
                            >
                                <i
                                    className={`fa-duotone fa-regular ${role.icon} text-[10px] ${accent.text}`}
                                />
                            </div>
                            <span className="flex-1 text-xs font-bold text-dark/60">
                                {role.label}
                            </span>
