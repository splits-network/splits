...
export default function PayoutSchedulesPage() {
    ...
    const defaultFilters = useMemo<ScheduleFilters>(
        () => ({ status: "scheduled" }),
        [],
    );
    ...
    const {
        ...
        fetchFn: async (params) => {
            ...
            if (params.filters?.trigger_event)
                queryParams.set("trigger_event", params.filters.trigger_event);
            if (params.sort_by) queryParams.set("sort_by", params.sort_by);
            if (params.sort_order)
                queryParams.set("sort_order", params.sort_order);

            const response = await apiClient.get(
                `/payout-schedules?${queryParams.toString()}`,
            );
            return response;
        },
        defaultFilters,
    ...
    function TriggerEventBadge({ event }: { event: string }) {
        ...
        return (
            <span className="badge badge-ghost">{labels[event] || event}</span>
        );
    }
    ...
    return (
        <div className="space-y-6">
                        ...
                        <fieldset className="fieldset">
                            ...
                            <select
                                ...
                                onChange={(e) =>
                                    setFilters({
                                        ...filters,
                                        trigger_event: e.target.value as any,
                                    })
                                }
                            >
                                ...
                                <option value="eligible_payout_catchup">
                                    Catchup
                                </option>
            ...
            >
                <p>
                    {confirmAction?.type === "trigger" &&
                        "Manually trigger this schedule? This will process it immediately."}
                    {confirmAction?.type === "cancel" &&
                        "Cancel this schedule? This action cannot be undone."}
                    {confirmAction?.type === "retry" &&
                        "Retry this failed schedule? This will reset it to pending status."}
                </p>
            </BaselConfirmModal>
        </div>
    );
