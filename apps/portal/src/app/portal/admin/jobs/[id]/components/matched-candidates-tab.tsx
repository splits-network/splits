...
export default function MatchedCandidatesTab({ jobId }: { jobId: string }) {
    ...
    if (loading) return <LoadingState message="Loading matched candidates..." />;
...
function CandidateMatchCard({ match, isPartner }: { match: EnrichedMatch; isPartner: boolean }) {
        ...
        <div className="card bg-base-100 border-2 border-base-200 hover:border-primary/30 transition-colors">
            <div className="card-body p-4 gap-3">
                ...
                <div className="flex items-center justify-between">
                    ...
                    {scoreLabel && (
                        <span className={`badge ${scoreLabel.badgeClass} badge-sm`}>
                            <i className={`fa-duotone fa-regular ${scoreLabel.icon} mr-1`} />
                            {scoreLabel.label}
                        </span>
                    )}
                </div>
                ...
                <div className="flex flex-wrap gap-1.5">
                    <FactorChip pass={match.match_factors.salary_overlap} label="Salary" />
                    <FactorChip pass={match.match_factors.employment_type_match} label="Type" />
                    <FactorChip pass={match.match_factors.location_compatible} label="Location" />
                    <FactorChip pass={match.match_factors.job_level_match} label="Level" />
                </div>

                {/* Skills */}
                {match.match_factors.skills_matched?.length > 0 && (
                    <div className="flex flex-wrap gap-1">
                        {match.match_factors.skills_matched.slice(0, 4).map((skill) => (
                            <span key={skill} className="badge badge-sm badge-success badge-outline">
                                {skill}
                            </span>
                        ))}
                ...
                {!isPartner && match.match_tier === 'standard' && (
                    <div className="flex items-center gap-2 p-2 bg-base-200 rounded-lg">
                        ...
                        <span className="text-sm text-base-content/50">True Score available</span>
                        <Link href="/portal/billing" className="link link-primary text-sm ml-auto">
                            Upgrade Plan
                        </Link>
                    </div>
                )}
...
function FactorChip({ pass, label }: { pass: boolean; label: string }) {
    return (
        <span className={`badge badge-sm ${pass ? 'badge-success badge-outline' : 'badge-error badge-outline'}`}>
            <i className={`fa-duotone fa-regular ${pass ? 'fa-check' : 'fa-xmark'} mr-1`} />
            {label}
        </span>
    );
}
