...
export default function JobDetailClient({ jobId }: { jobId: string }) {
    ...
    const loadJob = useCallback(async () => {
        try {
            ...
            const response = await client.get(`/jobs/${jobId}?include=company,requirements`);
    ...
    if (error || !job) {
        return (
            <div className="alert alert-error">
                ...
                <span>{error || 'This role could not be found. It may have been removed or is no longer available.'}</span>
            </div>
        );
    }
    ...
    const statusColors: Record<string, string> = {
        ...
        paused: 'badge-warning', closed: 'badge-neutral', filled: 'badge-info',
    };
    ...
    return (
        <div className="space-y-6">
            <AdminPageHeader
                ...
                badge={<span className={`badge ${statusColors[job.status] || 'badge-ghost'}`}>{job.status}</span>}
            />
