...
export function useHiringPipeline(trendPeriod: number = 12) {
    ...
    const refresh = useCallback(async () => {
        ...
        try {
            ...
            const response: any = await api.get('/charts/hiring-pipeline', {
                params: {
                    scope: 'company',
            ...
            if (chartData?.labels && chartData?.datasets?.[0]?.data) {
                const mapped: HiringStage[] = chartData.labels.map((label: string, i: number) => ({
                    label,
            ...
            console.error('[HiringPipeline] Failed to load:', err);
