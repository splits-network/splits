...
export default function RecruitmentFunnel({ trendPeriod = 12 }: RecruitmentFunnelProps) {
    const { stages, loading, error } = useFunnelData(trendPeriod);

    if (loading) {
        return (
            <MemphisCard title="Recruitment Funnel" icon="fa-filter" accent={ACCENT[1]} className="h-full">
