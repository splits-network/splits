...
export default function RoleDistribution({ stats, loading }: RoleDistributionProps) {
    if (loading) {
        return (
            <MemphisCard title="Role Distribution" icon="fa-chart-pie" accent={ACCENT[3]}>
    ...
    return (
        <MemphisCard title="Role Distribution" icon="fa-chart-pie" accent={ACCENT[3]}>
            ...
            <div className="text-center mb-5">
                <div className="text-3xl font-black tabular-nums text-dark">{total.toLocaleString()}</div>
                <div className="text-[10px] font-black uppercase tracking-widest text-dark/40">Total Roles</div>
            </div>
                ...
                {segments.map((seg) => {
                    ...
                    return (
                        <div key={seg.label} className="flex items-center gap-2">
                            ...
                            <span className="text-xs font-black tabular-nums text-dark ml-auto">{seg.count.toLocaleString()}</span>
                        </div>
                    );
                })}
            </div>
