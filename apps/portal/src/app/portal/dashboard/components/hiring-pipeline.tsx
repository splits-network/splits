...
export default function HiringPipeline({ trendPeriod = 12, refreshKey }: HiringPipelineProps) {
    const { stages, loading, error } = useHiringPipeline(trendPeriod);

    if (loading) {
        return (
            <ContentCard title="Hiring pipeline" icon="fa-filter" className="bg-base-200 h-full">
        ...
        return (
            <ContentCard title="Hiring pipeline" icon="fa-filter" className="bg-base-200 h-full">
                <EmptyState
                    ...
                    description="Post a role and recruiters will start submitting candidates. Your hiring funnel will appear here."
                    size="sm"
                />
            </ContentCard>
        );
                        ...
                        <div key={stage.label}>
                            <div className="flex items-center gap-3">
                                <div className="w-24 shrink-0 flex items-center gap-2 justify-end">
                                    ...
                                    <div className={`w-7 h-7 rounded-md ${config.bgLight} flex items-center justify-center shrink-0`}>
                                        <i className={`fa-duotone fa-regular ${icon} text-xs ${config.text}`}></i>
                                    </div>
                                </div>

                                <div className="flex-1 relative">
                                    <div
                                        className={`h-10 rounded-lg ${config.bg} transition-all duration-700 ease-out flex items-center justify-between px-3 min-w-[3rem]`}
                                        style={{ width: `${widthPercent}%` }}
                                    >
                                        <span className={`text-sm font-bold tabular-nums ${config.barText}`}>
