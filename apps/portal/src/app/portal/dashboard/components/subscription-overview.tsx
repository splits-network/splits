...
export default function SubscriptionOverview({ financials, loading }: SubscriptionOverviewProps) {
    if (loading) {
        return (
            <MemphisCard title="Subscriptions" icon="fa-credit-card" accent={ACCENT[3]} className="h-full">
    ...
    const segments = [
        { label: 'Active', count: financials.active_subscriptions, accentIdx: 1 },
        { label: 'Trialing', count: financials.trialing_subscriptions, accentIdx: 3 },
        { label: 'Past due', count: financials.past_due_subscriptions, accentIdx: 2 },
        { label: 'Canceled', count: financials.canceled_subscriptions, accentIdx: 0 },
    ];
    ...
    return (
        <MemphisCard title="Subscriptions" icon="fa-credit-card" accent={ACCENT[3]} className="h-full">
            ...
            <div className="flex items-center justify-between mb-4">
                ...
                <span className="text-2xl font-black tabular-nums text-dark">{totalSubs.toLocaleString()}</span>
            </div>

            {/* Stacked bar */}
            {totalSubs > 0 ? (
                <div className="h-5 border-4 border-dark flex overflow-hidden mb-4">
                    {segments.map((seg) => {
                ...
                <div className="h-5 border-4 border-dark bg-dark/5 mb-4" />
            )}
                        ...
                        <div key={seg.label}>
                            <div className="flex items-center justify-between mb-1">
                                <div className="flex items-center gap-2">
                                    ...
                                    <span className="text-[10px] font-bold text-dark/60">{seg.label}</span>
                                </div>
                                <div className="flex items-center gap-2">
                            ...
                            <div className="h-2 border-4 border-dark overflow-hidden bg-dark/5">
                                <div
