...
import { InterviewTranscriptPanel } from './interview-transcript-panel';
...
export function InterviewRecordingCard({
    ...
    // Fetch transcript when status is complete
    useEffect(() => {
        if (transcriptStatus !== 'complete') return;
        let cancelled = false;
        (async () => {
            try {
                ...
                const res = await client.get(`/interviews/${interviewId}/transcript`);
                if (!cancelled && res.data?.segments) {
                    setSegments(res.data.segments);
                }
            } catch {
                // silent - transcript display is non-critical
            } finally {
    ...
    return (
        <div className="flex flex-col gap-0">
            ...
            {transcriptStatus && transcriptStatus !== 'complete' && (
                <AIPipelineStatus status={transcriptStatus} />
            )}
