...
export default function ApplicationDetailClient({
    ...
    const loadApplicationData = useCallback(async () => {
        ...
        try {
            ...
            const appResponse: any = await client.get(
                `/applications/${applicationId}?include=job,documents,pre_screen_answers,job_requirements`,
            );
            ...
            try {
                ...
                const response: any = await client.get(
                    `/candidates/${appData.candidate_id}`,
                );
            ...
            try {
                const auditLogResponse: any = await client.get(
                    `/applications/${applicationId}?include=audit_log`,
                );
            ...
            if (isRecruiter) {
                ...
                try {
                    const recruiterResponse: any =
                        await client.getCurrentRecruiter();
                    ...
                    if (appData.candidate && recruiterProfile) {
                        try {
                            const relationshipResponse: any = await client.get(
                                `/recruiter-candidates?recruiter_id=${recruiterProfile.id}&candidate_id=${appData.candidate.id}&limit=1`,
                            );
                            const relationships =
                ...
                if (organizationId) {
                    ...
                    if (
                        appData.job?.company?.identity_organization_id !==
                        organizationId
                    ) {
                        setError(
                            "You do not have permission to view this application",
            ...
            if (acceptedByCompany) {
                ...
                toast.success(
                    targetStage === "hired"
                        ...
                        : "Application moved to next stage successfully",
                );
            }
            await loadApplicationData();
        } catch (error: any) {
    ...
    const canUploadCompanyDocs =
        isCompanyUser && ["offer", "hired"].includes(application?.stage);
                        ...
                        <div className="alert alert-warning">
                            ...
                            <div>
                                ...
                                <div className="text-sm">
                                    Your relationship with this candidate is{" "}
                                    {relationship.status}. Historical data is
                                    preserved but you may have limited editing
                                    capabilities.
                                </div>
                            </div>
                        </div>
                    ...
                    <div className="card-body">
                        ...
                        <div className="space-y-3">
                            <div>
                                ...
                                <span
                                    className={`badge ${getApplicationStageBadge(application.stage)} badge-lg`}
                                >
                                    {
                        ...
                        <div className="card-body">
                            ...
                            <div className="space-y-2">
                                {application.documents.map((doc: any) => (
                                    <div
                                        key={doc.id}
                                        className="flex items-center justify-between p-3 rounded-lg bg-base-300 hover:bg-base-300 transition-colors"
                                    >
                        ...
                        <div className="card-body">
                            ...
                            <div className="space-y-2 text-sm">
                                <div>
                                    ...
                                    <span
                                        className={`badge ${relationship.status === "active" ? "badge-success" : "badge-neutral"}`}
                                    >
                                        {relationship.status}
                                    </span>
                                </div>
