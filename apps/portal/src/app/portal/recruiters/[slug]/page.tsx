...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
    ...
    try {
        const response = await apiClient.get<{ data: RecruiterWithUser }>(
            `/recruiters/by-slug/${slug}`,
        ...
        return {
            ...
            openGraph: {
                title,
                description,
                url: `${PORTAL_BASE_URL}/portal/recruiters/${slug}`,
            },
            ...buildCanonical(`/portal/recruiters/${slug}`),
        };
    } catch {
...
export default async function RecruiterProfilePage({ params }: Props) {
    ...
    const jsonLd = {
        ...
        ...(recruiter.location
            ...
            : {}),
        ...(recruiter.bio ? { description: recruiter.bio.substring(0, 160) } : {}),
    };
