...
import { PanelHeader, PanelTabs, type PanelStat } from "@splits-network/basel-ui";
...
import { buildFinancialsTable, copyToClipboard, downloadCsv } from "./financials-export";
...
export function PlacementDetailPanel({
    ...
    const headerBadges = [
        { label: formatStatus(state), className: STATUS_BADGE_CLASS[statusColor] || "badge-ghost" },
        ...(isNew(placement)
    ...
    const meta = [
        ...
        ...(placement.candidate?.email
            ? [{ icon: "fa-duotone fa-regular fa-envelope", text: placement.candidate.email }]
            : []),
    ];

    /* Stats strip */
    const stats: PanelStat[] = [
        { label: "Salary", value: formatCurrencyShort(placement.salary || 0), icon: "fa-duotone fa-regular fa-dollar-sign" },
        { label: "Fee Rate", value: `${placement.fee_percentage || 0}%`, icon: "fa-duotone fa-regular fa-percent" },
        { label: "Your Share", value: formatCurrencyShort(placement.recruiter_share || 0), icon: "fa-duotone fa-regular fa-coins" },
        ...(placement.guarantee_days != null
            ? [{ label: "Guarantee", value: `${placement.guarantee_days}d`, icon: "fa-duotone fa-regular fa-shield-check" }]
            : []),
    ];
    ...
    return (
        <div className="flex flex-col h-full min-h-0 bg-base-100">
            <PanelHeader
                kicker={placement.hired_at ? `Placed ${formatDate(placement.hired_at)}` : ""}
                badges={headerBadges}
                avatar={{ initials }}
                title={name}
                subtitle={company}
            ...
            <PanelTabs tabs={TABS} defaultTab="financials">
                {(tab) => {
                    if (tab === "financials") return <FinancialsTab placement={placement} placementId={placement.id} />;
                    if (tab === "dates") return <DatesTab placement={placement} />;
...
function FinancialsTab({ placement, placementId }: { placement: Placement; placementId: string }) {
    const state = placement.state || "unknown";
    ...
    const handleCopy = async () => {
        const table = buildFinancialsTable(placement, billing.invoice, billing.payoutTransactions);
    ...
    const handleDownload = () => {
        const table = buildFinancialsTable(placement, billing.invoice, billing.payoutTransactions);
        const name = candidateName(placement).replace(/\s+/g, "-").toLowerCase();
    ...
    return (
        <div className="flex-1 min-h-0 overflow-y-auto space-y-8 p-6">
            ...
            <div className="flex items-center gap-2 justify-end">
                <button onClick={handleCopy} className="btn btn-sm btn-ghost text-base-content/50" title="Copy to clipboard (Excel-friendly)">
                    <i className={`fa-duotone fa-regular ${copied ? "fa-check" : "fa-clipboard"} mr-1`} />
                    {copied ? "Copied!" : "Copy"}
                </button>
                <button onClick={handleDownload} className="btn btn-sm btn-ghost text-base-content/50" title="Download CSV">
            ...
            <div className="grid grid-cols-3 gap-[2px] bg-base-300">
                {[
                    ...
                    { l: "Your Share", v: formatCurrency(recruiterShare), accent: true },
                ].map((s) => (
            ...
            <div className="border-l-4 border-l-primary pl-6">
                ...
                {[
                    { l: "Total Fee", v: formatCurrency(feeAmount), bold: true },
            ...
            {placement.guarantee_days != null && (
                <div>
                    ...
                    <div className="bg-primary/5 p-4">
                        ...
                        <p className="text-base font-bold text-primary">{placement.guarantee_days} days</p>
                    </div>
                </div>
            )}
            ...
            {state === "failed" && placement.failed_at && (
                <div className="border-t-2 border-base-300 pt-6">
                    <p className="text-xs font-bold uppercase tracking-[0.22em] text-error mb-3">Placement Failed</p>
                    <p className="text-sm text-base-content/60">Failed on {formatDate(placement.failed_at)}</p>
                </div>
            )}
            ...
            {billing.loading ? (
                <div className="flex items-center gap-3 py-4">
                    ...
                    <span className="text-sm text-base-content/40">Loading billing details...</span>
                </div>
            ) : (
                <>
                    <InvoiceSection invoice={billing.invoice} />
                    <PayoutTransactionsSection transactions={billing.payoutTransactions} />
                    <PayoutScheduleSection schedules={billing.payoutSchedules} />
...
function DatesTab({ placement }: { placement: Placement }) {
    const dates = [
        ...
        ...(placement.start_date ? [{ l: "Start Date", v: formatDate(placement.start_date) }] : []),
        ...(placement.end_date ? [{ l: "End Date", v: formatDate(placement.end_date) }] : []),
        ...(placement.guarantee_expires_at ? [{ l: "Guarantee Expires", v: formatDate(placement.guarantee_expires_at) }] : []),
    ];
    ...
    const info = [
        ...
        ...(placement.candidate?.email ? [{ l: "Email", v: placement.candidate.email }] : []),
    ];
        ...
        <div className="flex-1 min-h-0 overflow-y-auto space-y-8 p-6">
            {placement.splits && placement.splits.length > 0 ? (
                <div>
                    ...
                    <ul className="space-y-2">
                        {placement.splits.map((split) => {
                            ...
                            return (
                                <li key={split.id} className="flex items-center justify-between text-base-content/70 border-b border-base-200 pb-2">
                                    ...
                                    <span className="text-sm font-bold">
                                        {split.split_percentage}% &middot; {formatCurrency(split.split_amount)}
                                    </span>
                                </li>
                            );
                        })}
                ...
                <div className="h-full flex items-center justify-center p-12">
                    <div className="text-center">
                        ...
                        <h3 className="text-lg font-black tracking-tight mb-2">No Split Partners</h3>
                        <p className="text-sm text-base-content/40">This placement has no split arrangements.</p>
                    </div>
                </div>
            )}
            ...
            <div className="bg-base-200 border border-base-300 p-4">
                <div className="flex justify-between text-sm mb-1">
                    ...
                    <span className="font-bold text-primary">{formatCurrency(feeAmount)}</span>
                </div>
                <div className="flex justify-between text-sm">
                    <span className="text-base-content/50">Fee Rate</span>
                    <span className="font-semibold">{feePercentage}% of {formatCurrency(salary)}</span>
                </div>
            </div>
        </div>
    );
