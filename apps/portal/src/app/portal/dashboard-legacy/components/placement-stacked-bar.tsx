        ...
        <div>
            ...
            <div className="flex flex-wrap gap-3 mb-3">
                {chartData.datasets.map((ds, i) => (
                    <div key={ds.label} className="flex items-center gap-1.5">
                        <div
                            className={`w-3 h-3 border-2 border-dark ${BAR_CLASSES[i % BAR_CLASSES.length]}`}
                        />
            ...
            <div className="flex items-end gap-1 h-28">
                {chartData.labels.map((label, colIdx) => {
                    ...
                    const heightPct =
                        maxStacked > 0 ? (colTotal / maxStacked) * 100 : 0;

                    return (
                        <div
                            key={label}
                            className="flex-1 flex flex-col items-stretch"
                            title={`${label}: ${colTotal}`}
                        >
                            {/* Stacked segments */}
                            <div
                                className="flex flex-col-reverse border-1 border-dark overflow-hidden transition-all duration-500"
                            ...
                            {/* Label */}
                            <div className="text-xs font-bold text-dark/30 text-center mt-1 uppercase truncate">
                                {label}
                            </div>
                        </div>
                    );
