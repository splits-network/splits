...
interface MessagesAnimatorProps {
    children: ReactNode;
}

export function MessagesAnimator({ children }: MessagesAnimatorProps) {
    ...
    useGSAP(
        () => {
            ...
            // ════════════════════════════════════════
            // THREAD CARDS -- stagger slide-right
            // ════════════════════════════════════════
            const threadContainer = $1(".thread-container");
            if (threadContainer) {
                gsap.fromTo(
                    $(".thread-card"),
                    { opacity: 0, x: -25 },
                    {
                        ...
                        ease: E.gentle,
                        stagger: S.normal,
                        scrollTrigger: { trigger: threadContainer, start: "top 75%" },
                    },
                );
            }
            ...
            if (notificationContainer) {
                gsap.fromTo(
                    ...
                    { opacity: 0, y: 20 },
                    {
                        ...
                        ease: E.gentle,
                        stagger: S.normal,
                        scrollTrigger: { trigger: notificationContainer, start: "top 80%" },
                    },
                );
            }

            // ════════════════════════════════════════
            // FEATURE CARDS (mentions, attachments, etc.)
            // ════════════════════════════════════════
            const featureContainer = $1(".feature-container");
            if (featureContainer) {
                gsap.fromTo(
                    $(".feature-card"),
                    { opacity: 0, y: 25, scale: 0.95 },
                    {
                        ...
                        ease: E.gentle,
                        stagger: S.normal,
                        scrollTrigger: { trigger: featureContainer, start: "top 80%" },
                    },
                );
            }
            ...
            // ARCHIVE CARDS
            // ════════════════════════════════════════
            const archiveContainer = $1(".archive-container");
            if (archiveContainer) {
                gsap.fromTo(
                    $(".archive-card"),
                    { opacity: 0, x: -15 },
                    {
                        ...
                        ease: E.smooth,
                        stagger: S.tight,
                        scrollTrigger: { trigger: archiveContainer, start: "top 75%" },
                    },
                );
            }
            ...
            // CTA / NEXT STEPS
            // ════════════════════════════════════════
            const ctaSection = $1(".messages-cta");
            if (ctaSection) {
