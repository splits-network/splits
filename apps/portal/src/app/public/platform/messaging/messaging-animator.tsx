...
export function MessagingAnimator({ children }: { children: ReactNode }) {
    ...
    useGSAP(
        () => {
            ...
            if (featuresHeading) gsap.fromTo(featuresHeading, { opacity: 0, y: 40 }, { opacity: 1, y: 0, duration: D.normal, scrollTrigger: { trigger: $1(".messaging-features"), start: "top 75%" } });
            ...
            if (benefitBlocks.length) gsap.fromTo(benefitBlocks, { opacity: 0, scale: 0.8 }, { opacity: 1, scale: 1, duration: D.normal, ease: E.bounce, stagger: S.normal, scrollTrigger: { trigger: $1(".messaging-benefits"), start: "top 85%" } });

            const useCasesHeading = $1(".use-cases-heading");
            if (useCasesHeading) gsap.fromTo(useCasesHeading, { opacity: 0, y: 40 }, { opacity: 1, y: 0, duration: D.normal, scrollTrigger: { trigger: $1(".messaging-use-cases"), start: "top 75%" } });
            ...
            if (ctaContent) gsap.fromTo(ctaContent, { opacity: 0, y: 50 }, { opacity: 1, y: 0, duration: D.hero, scrollTrigger: { trigger: $1(".messaging-cta"), start: "top 80%" } });
            const ctaButtons = $1(".cta-buttons");
            if (ctaButtons) gsap.fromTo(ctaButtons, { opacity: 0, y: 30 }, { opacity: 1, y: 0, duration: D.normal, ease: E.bounce, delay: 0.3, scrollTrigger: { trigger: $1(".messaging-cta"), start: "top 80%" } });
        },
        { scope: containerRef },
    );
