...
export function calculateApplicationTrends(
    ...
    const previousPeriodStart = new Date(
        now.getFullYear(),
        now.getMonth() - months * 2,
        1,
    );

    // Current period counts
    const currentPeriodApps = applications.filter(
        (app) => new Date(app.created_at) >= periodStart,
    );
    const currentTotal = currentPeriodApps.length;
    const currentPending = currentPeriodApps.filter(
        (a) => a.stage === "submitted" || a.stage === "screen",
    ).length;
    const currentAccepted = currentPeriodApps.filter(
        (a) => a.accepted_by_company,
    ).length;
    const currentOffer = currentPeriodApps.filter(
        (a) => a.stage === "offer",
    ).length;
    ...
    const previousPending = previousPeriodApps.filter(
        (a) => a.stage === "submitted" || a.stage === "screen",
    ).length;
    const previousAccepted = previousPeriodApps.filter(
        (a) => a.accepted_by_company,
    ).length;
    const previousOffer = previousPeriodApps.filter(
        (a) => a.stage === "offer",
    ).length;
...
export default function ApplicationsTrendsChart({
    ...
    const changes = useMemo(() => {
        ...
        return {
            total: calcPercentChange(
                totalApps[len - 1] || 0,
                totalApps[len - 2] || 0,
            ),
            pending: calcPercentChange(
                pendingApps[len - 1] || 0,
                pendingApps[len - 2] || 0,
            ),
            accepted: calcPercentChange(
                acceptedApps[len - 1] || 0,
                acceptedApps[len - 2] || 0,
            ),
            offer: calcPercentChange(
                offerApps[len - 1] || 0,
                offerApps[len - 2] || 0,
            ),
        };
        ...
        <div className="space-y-3">
                        ...
                        {TIME_PERIODS.map((period) => (
                            <li key={period.value}>
                                <button
                                    ...
                                    onClick={() =>
                                        onTrendPeriodChange(period.value)
                                    }
                                >
                                    {period.label}
                                </button>
            ...
            <div className="flex items-center gap-4 text-xs">
                ...
                <div className="flex items-center gap-1.5">
                    <span
                        className="w-2 h-2 rounded-full"
                        style={{ backgroundColor: dataset.warningBorderColor }}
                    ></span>
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center gap-6 pt-2 border-t border-base-300">
                <div className="flex items-center gap-2 text-xs">
                    ...
                    <span className="text-base-content/70">Total</span>
                    <span
                        className={`font-medium ${changes.total.startsWith("+") ? "text-success" : changes.total.startsWith("-") ? "text-error" : "text-base-content/50"}`}
                    >
                        {changes.total}
                    </span>
                </div>
                <div className="flex items-center gap-2 text-xs">
                    ...
                    <span className="text-base-content/70">Pending</span>
                    <span
                        className={`font-medium ${changes.pending.startsWith("+") ? "text-success" : changes.pending.startsWith("-") ? "text-error" : "text-base-content/50"}`}
                    >
                        {changes.pending}
                    </span>
                </div>
                <div className="flex items-center gap-2 text-xs">
                    ...
                    <span className="text-base-content/70">Accepted</span>
                    <span
                        className={`font-medium ${changes.accepted.startsWith("+") ? "text-success" : changes.accepted.startsWith("-") ? "text-error" : "text-base-content/50"}`}
                    >
                        {changes.accepted}
                    </span>
                </div>
                <div className="flex items-center gap-2 text-xs">
                    ...
                    <span className="text-base-content/70">Offers</span>
                    <span
                        className={`font-medium ${changes.offer.startsWith("+") ? "text-success" : changes.offer.startsWith("-") ? "text-error" : "text-base-content/50"}`}
                    >
                        {changes.offer}
                    </span>
                </div>
