...
export function calculateStatTrends(
    jobs: Job[],
    months: number,
): {
    total: number;
    ...
    const currentPeriodJobs = jobs.filter(
        (job) => new Date(job.created_at) >= periodStart,
    );
    const currentTotal = currentPeriodJobs.length;
    const currentActive = currentPeriodJobs.filter(
        (j) => j.status === "active",
    ).length;
    const currentPaused = currentPeriodJobs.filter(
        (j) => j.status === "paused",
    ).length;
    const currentFilled = currentPeriodJobs.filter(
        (j) => j.status === "filled",
    ).length;
    ...
    const previousActive = previousPeriodJobs.filter(
        (j) => j.status === "active",
    ).length;
    const previousPaused = previousPeriodJobs.filter(
        (j) => j.status === "paused",
    ).length;
    const previousFilled = previousPeriodJobs.filter(
        (j) => j.status === "filled",
    ).length;
...
export function RolesTrendsChart({
    jobs,
    loading,
    trendPeriod,
    onTrendPeriodChange,
}: RolesTrendsChartProps) {
    ...
    const changes = useMemo(() => {
        ...
        return {
            roles: calcPercentChange(
                rolesCreated[len - 1] || 0,
                rolesCreated[len - 2] || 0,
            ),
            active: calcPercentChange(
                activeRoles[len - 1] || 0,
                activeRoles[len - 2] || 0,
            ),
            filled: calcPercentChange(
                filledRoles[len - 1] || 0,
                filledRoles[len - 2] || 0,
            ),
        };
        ...
        <div className="space-y-3">
            ...
            <div className="flex items-center gap-4 text-xs">
                ...
                <div className="flex items-center gap-1.5">
                    <span
                        className="w-2 h-2 rounded-full"
                        style={{ backgroundColor: dataset.neutralBorderColor }}
                    ></span>
            ...
            <div className="flex items-center justify-center gap-6 pt-2 border-t border-base-300">
                <div className="flex items-center gap-2 text-xs">
                    ...
                    <span className="text-base-content/70">New Roles</span>
                    <span
                        className={`font-medium ${changes.roles.startsWith("+") ? "text-success" : changes.roles.startsWith("-") ? "text-error" : "text-base-content/50"}`}
                    >
                        {changes.roles}
                    </span>
                </div>
                <div className="flex items-center gap-2 text-xs">
                    ...
                    <span className="text-base-content/70">Active</span>
                    <span
                        className={`font-medium ${changes.active.startsWith("+") ? "text-success" : changes.active.startsWith("-") ? "text-error" : "text-base-content/50"}`}
                    >
                        {changes.active}
                    </span>
                </div>
                <div className="flex items-center gap-2 text-xs">
                    ...
                    <span className="text-base-content/70">Filled</span>
                    <span
                        className={`font-medium ${changes.filled.startsWith("+") ? "text-success" : changes.filled.startsWith("-") ? "text-error" : "text-base-content/50"}`}
                    >
                        {changes.filled}
                    </span>
                </div>
