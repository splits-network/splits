...
function TimelineStep({
    ...
    return (
        <div
            className={`timeline-step relative flex items-start gap-6 ${align === "right" ? "flex-row-reverse text-right" : ""}`}
        >
            {/* Timeline node and line */}
            <div className="flex flex-col items-center">
                {/* Number circle */}
                <div
                    className={`step-node relative  w-16 h-16 rounded-full ${colors.bg} text-white flex items-center justify-center shadow-lg ${colors.glow} shadow-xl`}
                >
    ...
    useGSAP(
        () => {
            ...
            if (recruiterTrackRef.current) {
                const recruiterSteps =
                    recruiterTrackRef.current.querySelectorAll(
                        ".timeline-step",
                    );
                const recruiterNodes =
                    recruiterTrackRef.current.querySelectorAll(".step-node");
                const recruiterLines =
                    recruiterTrackRef.current.querySelectorAll(
                        ".timeline-line",
                    );
                const recruiterContent =
                    recruiterTrackRef.current.querySelectorAll(".step-content");
            ...
            if (companyTrackRef.current) {
                const companyNodes =
                    companyTrackRef.current.querySelectorAll(".step-node");
                const companyLines =
                    companyTrackRef.current.querySelectorAll(".timeline-line");
                const companyContent =
                    companyTrackRef.current.querySelectorAll(".step-content");
