...
export interface RecruiterGrowthChartProps {
    data: { period: string; count: number }[];
    loading?: boolean;
    height?: number;
}

export function RecruiterGrowthChart({
    data,
    loading = false,
    height = 320,
}: RecruiterGrowthChartProps) {
    ...
    if (!data.length) {
        return <ChartLoadingState height={height} message="No recruiter data" />;
    }
