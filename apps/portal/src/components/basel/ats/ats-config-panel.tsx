...
export default function ATSConfigPanel({
    companyId,
    onClose,
}: ATSConfigPanelProps) {
    ...
    /* ── Sub-views ── */
    const [setupPlatform, setSetupPlatform] = useState<ATSPlatform | null>(
        null,
    );
    ...
    const availablePlatforms = (
        Object.keys(PLATFORM_META) as ATSPlatform[]
    ).filter((p) => p !== "generic" && !connectedPlatforms.has(p));
                                            ...
                                            return (
                                                ...
                                                >
                                                    <div className="w-10 h-10 bg-primary/10 border border-primary/20 flex items-center justify-center shrink-0">
                                                        <i
                                                            className={`${meta.icon} text-lg ${meta.color}`}
                                                        />
                                                    </div>
                                            ...
                                            return (
                                                ...
                                                >
                                                    <div className="w-10 h-10 bg-base-200 border border-base-300 flex items-center justify-center shrink-0">
                                                        <i
                                                            className={`${meta.icon} text-lg text-base-content/40`}
                                                        />
                                                    </div>
                                                    <div>
                                                        <p className="text-sm font-bold">
                                                        ...
                                                        <p className="text-sm text-base-content/40 uppercase tracking-wider font-semibold">
                                                            Connect
                                                        </p>
                                                    </div>
                                                </button>
    ...
    return (
        <div className="space-y-5">
            ...
            {(platform === "workable" || platform === "generic") && (
                <fieldset>
                    <legend className="text-xs font-bold uppercase tracking-wider text-base-content/50 mb-2">
                        {platform === "workable" ? "Subdomain" : "API Base URL"}{" "}
                        (optional)
                    </legend>
                    <input
                        type="text"
            ...
            <div className="bg-base-200/50 border border-base-300 p-4 flex items-start gap-3">
                ...
                <div>
                    ...
                    <p className="text-xs text-base-content/40 leading-relaxed">
                        Your API key is encrypted at rest and never exposed in
                        the frontend. We&apos;ll validate the connection before
                        saving.
                    </p>
                </div>
            </div>
...
function IntegrationDetail({
        ...
        (async () => {
            try {
                ...
                const res = (await client.get(
                    `/integrations/ats/${integration.id}/stats`,
                )) as { data: SyncStats };
                setStats(res.data);
            } catch {
                // non-critical
    ...
    const toggleSetting = async (field: string, value: boolean) => {
        ...
        try {
            ...
            const res = (await client.patch(
                `/integrations/ats/${integration.id}`,
                {
                    [field]: value,
                },
            )) as { data: ATSIntegrationPublic };
    ...
    const loadLogs = async () => {
        ...
        try {
            ...
            const client = createAuthenticatedClient(token);
            const res = (await client.get(
                `/integrations/ats/${integration.id}/sync-logs`,
                {
                    params: { limit: "20" },
                },
    ...
    const SYNC_TOGGLES = [
        {
            field: "sync_enabled",
            label: "Master Sync",
            description: "Enable or disable all syncing",
            icon: "fa-duotone fa-regular fa-power-off",
        },
        {
            field: "sync_roles",
            label: "Roles / Jobs",
            description: "Sync job postings from your ATS",
            icon: "fa-duotone fa-regular fa-briefcase",
        },
        {
            field: "sync_candidates",
            label: "Candidates",
            description: "Sync candidate profiles",
            icon: "fa-duotone fa-regular fa-users",
        },
        {
            field: "sync_applications",
            label: "Applications",
            description: "Sync application pipeline data",
            icon: "fa-duotone fa-regular fa-file-lines",
        },
        {
            field: "sync_interviews",
            label: "Interviews",
