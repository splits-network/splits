...
CREATE TABLE IF NOT EXISTS candidate_sourcers (
    ...
    sourcer_user_id UUID NOT NULL REFERENCES users(id) ON DELETE CASCADE,
...
CREATE INDEX idx_candidate_sourcers_candidate ON candidate_sourcers(candidate_id);
CREATE INDEX idx_candidate_sourcers_sourcer ON candidate_sourcers(sourcer_user_id);
CREATE INDEX idx_candidate_sourcers_expires ON candidate_sourcers(protection_expires_at);

COMMENT ON TABLE candidate_sourcers IS 'Tracks who first sourced each candidate and protection windows';
COMMENT ON COLUMN candidate_sourcers.sourcer_type IS 'Who sourced: recruiter or tsn (The Splits Network)';
COMMENT ON COLUMN candidate_sourcers.protection_window_days IS 'How long the sourcer has attribution rights';
...
CREATE TABLE IF NOT EXISTS candidate_role_assignments (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE INDEX idx_cra_response_due ON candidate_role_assignments(response_due_at);

COMMENT ON TABLE candidate_role_assignments IS 'State machine for candidate-job proposals and collaboration';
COMMENT ON COLUMN candidate_role_assignments.state IS 'Current state in proposal lifecycle';
COMMENT ON COLUMN candidate_role_assignments.response_due_at IS 'When the proposal times out if no response';
...
ALTER TABLE placements 
    ...
    ADD COLUMN IF NOT EXISTS replacement_placement_id UUID REFERENCES placements(id);

CREATE INDEX idx_placements_state ON placements(state);
CREATE INDEX idx_placements_guarantee_expires ON placements(guarantee_expires_at);

COMMENT ON COLUMN placements.state IS 'Placement lifecycle: hired -> active -> completed or failed';
COMMENT ON COLUMN placements.guarantee_days IS 'Guarantee period in days (default 90)';
COMMENT ON COLUMN placements.replacement_placement_id IS 'If this placement replaced a failed one';

-- Track multiple recruiters per placement with explicit split percentages
CREATE TABLE IF NOT EXISTS placement_collaborators (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE INDEX idx_placement_collaborators_placement ON placement_collaborators(placement_id);
CREATE INDEX idx_placement_collaborators_recruiter ON placement_collaborators(recruiter_user_id);

COMMENT ON TABLE placement_collaborators IS 'Multiple recruiters can collaborate on a placement with explicit splits';
COMMENT ON COLUMN placement_collaborators.role IS 'What this recruiter contributed';
COMMENT ON COLUMN placement_collaborators.split_percentage IS 'Their share of the recruiter portion';
...
CREATE TABLE IF NOT EXISTS recruiter_reputation (
...
CREATE INDEX idx_recruiter_reputation_score ON recruiter_reputation(reputation_score DESC);
CREATE INDEX idx_recruiter_reputation_hire_rate ON recruiter_reputation(hire_rate DESC);

COMMENT ON TABLE recruiter_reputation IS 'Aggregated reputation metrics for each recruiter';
COMMENT ON COLUMN recruiter_reputation.reputation_score IS 'Overall quality score (0-100) derived from outcomes';
...
CREATE INDEX idx_outreach_candidate ON candidate_outreach(candidate_id);
CREATE INDEX idx_outreach_recruiter ON candidate_outreach(recruiter_user_id);
CREATE INDEX idx_outreach_job ON candidate_outreach(job_id);
CREATE INDEX idx_outreach_sent ON candidate_outreach(sent_at DESC);

COMMENT ON TABLE candidate_outreach IS 'Tracks all outreach emails sent to candidates via Resend';
COMMENT ON COLUMN candidate_outreach.sent_at IS 'First outreach establishes ownership';
...
CREATE INDEX idx_marketplace_events_user ON marketplace_events(user_id);
CREATE INDEX idx_marketplace_events_recruiter ON marketplace_events(recruiter_id);
CREATE INDEX idx_marketplace_events_created ON marketplace_events(created_at DESC);

COMMENT ON TABLE marketplace_events IS 'Event log for marketplace actions (analytics and audit trail)';
...
CREATE OR REPLACE FUNCTION initialize_recruiter_reputation()
RETURNS TRIGGER AS $$
BEGIN
    INSERT INTO recruiter_reputation (recruiter_id)
    VALUES (NEW.id)
...
CREATE TRIGGER trigger_initialize_reputation
    ...
    FOR EACH ROW
    EXECUTE FUNCTION initialize_recruiter_reputation();
...
CREATE OR REPLACE FUNCTION calculate_protection_expiry(
...
CREATE OR REPLACE FUNCTION is_candidate_protected(
    p_candidate_id UUID,
...
COMMENT ON FUNCTION is_candidate_protected IS 'Check if a candidate is protected (and by whom)';
