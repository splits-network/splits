...
CREATE TABLE IF NOT EXISTS placement_splits (
    ...
    split_configuration_id UUID REFERENCES split_configurations(id) ON DELETE SET NULL,
    notes TEXT,
    created_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
);
...
CREATE INDEX idx_team_members_recruiter ON team_members(recruiter_id);
CREATE INDEX idx_team_members_status ON team_members(status);
CREATE INDEX idx_split_configs_team ON split_configurations(team_id);
CREATE INDEX idx_split_configs_default ON split_configurations(team_id, is_default) WHERE is_default = TRUE;
CREATE INDEX idx_placement_splits_placement ON placement_splits(placement_id);
CREATE INDEX idx_placement_splits_team ON placement_splits(team_id);
CREATE INDEX idx_placement_splits_recruiter ON placement_splits(recruiter_id);
...
GRANT SELECT, INSERT, UPDATE, DELETE ON split_configurations TO authenticated;
GRANT SELECT, INSERT, UPDATE, DELETE ON placement_splits TO authenticated;
...
ALTER TABLE placement_splits ENABLE ROW LEVEL SECURITY;
...
CREATE POLICY teams_select_policy ON teams
    ...
    USING (
        ...
        OR id IN (
            ...
            JOIN recruiters r ON r.id = tm.recruiter_id
...
COMMENT ON TABLE teams IS 'Recruiting agencies and team accounts that can operate as unified entities';
COMMENT ON TABLE team_members IS 'Membership relationships between recruiters and teams';
COMMENT ON TABLE split_configurations IS 'Economic split models for team-based fee distribution';
COMMENT ON TABLE placement_splits IS 'Individual split distributions for each placement';
COMMENT ON TABLE team_invitations IS 'Pending invitations for recruiters to join teams';
