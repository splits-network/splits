...
CREATE INDEX IF NOT EXISTS documents_entity_type_id_idx ON documents(entity_type, entity_id) WHERE deleted_at IS NULL;
CREATE INDEX IF NOT EXISTS documents_document_type_idx ON documents(document_type) WHERE deleted_at IS NULL;
CREATE INDEX IF NOT EXISTS documents_uploaded_by_user_id_idx ON documents(uploaded_by_user_id) WHERE deleted_at IS NULL;
CREATE INDEX IF NOT EXISTS documents_processing_status_idx ON documents(processing_status) WHERE deleted_at IS NULL;
CREATE INDEX IF NOT EXISTS documents_created_at_idx ON documents(created_at DESC) WHERE deleted_at IS NULL;
...
-- Grant permissions
GRANT SELECT, INSERT, UPDATE, DELETE ON documents TO postgres, service_role;
...
CREATE POLICY documents_authenticated_update_own ON documents
    FOR UPDATE
    TO authenticated
...
COMMENT ON TABLE documents IS 'Stores metadata for documents uploaded to the platform (resumes, contracts, verification documents, etc.)';
