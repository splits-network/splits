...
-- Add comment
COMMENT ON TABLE ai_reviews IS 'AI-generated analysis of candidate-job fit for applications';
COMMENT ON COLUMN ai_reviews.fit_score IS 'AI fit score from 0-100, higher is better match';
COMMENT ON COLUMN ai_reviews.recommendation IS 'AI recommendation category: strong_fit, good_fit, fair_fit, poor_fit';
COMMENT ON COLUMN ai_reviews.confidence_level IS 'AI confidence in analysis from 0-100';
...
-- Create index
CREATE INDEX IF NOT EXISTS idx_applications_ai_reviewed ON applications(ai_reviewed);

-- Add comment
COMMENT ON COLUMN applications.ai_reviewed IS 'Whether AI review has been completed for this application';
...
-- GRANT SELECT, INSERT, UPDATE, DELETE ON ai_reviews TO ats_service_role;
-- GRANT USAGE, SELECT ON SEQUENCE ai_reviews_id_seq TO ats_service_role;
...
-- Drop indexes
DROP INDEX IF EXISTS idx_applications_ai_reviewed;
DROP INDEX IF EXISTS idx_ai_reviews_analyzed_at;
DROP INDEX IF EXISTS idx_ai_reviews_recommendation;
DROP INDEX IF EXISTS idx_ai_reviews_fit_score;
DROP INDEX IF EXISTS idx_ai_reviews_application_id;

-- Drop column
ALTER TABLE applications DROP COLUMN IF EXISTS ai_reviewed;
