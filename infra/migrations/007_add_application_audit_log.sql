...
CREATE INDEX IF NOT EXISTS idx_audit_log_application_id ON ats.application_audit_log(application_id);
CREATE INDEX IF NOT EXISTS idx_audit_log_action ON ats.application_audit_log(action);
CREATE INDEX IF NOT EXISTS idx_audit_log_created_at ON ats.application_audit_log(created_at DESC);
CREATE INDEX IF NOT EXISTS idx_audit_log_company_id ON ats.application_audit_log(company_id) WHERE company_id IS NOT NULL;
...
COMMENT ON COLUMN ats.application_audit_log.action IS 
'Type of action: created, accepted, rejected, stage_changed, viewed, etc.';

COMMENT ON COLUMN ats.application_audit_log.performed_by_user_id IS 
'User ID from identity service who performed the action';

COMMENT ON COLUMN ats.application_audit_log.performed_by_role IS 
'Role of the user who performed the action (company_admin, hiring_manager, recruiter, platform_admin)';

COMMENT ON COLUMN ats.application_audit_log.company_id IS 
'Company ID associated with the action (for company-side actions)';

COMMENT ON COLUMN ats.application_audit_log.old_value IS 
'Previous state before the action (for updates/changes)';

COMMENT ON COLUMN ats.application_audit_log.new_value IS 
'New state after the action (for updates/changes)';

COMMENT ON COLUMN ats.application_audit_log.metadata IS 
'Additional context like candidate_id, job_id, recruiter_id, notes, etc.';

COMMENT ON COLUMN ats.application_audit_log.ip_address IS 
'IP address of the user who performed the action (for security tracking)';

COMMENT ON COLUMN ats.application_audit_log.user_agent IS 
