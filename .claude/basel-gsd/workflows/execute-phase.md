...
**No dependency analysis needed.** Wave numbers are pre-computed during `/basel:plan-phase`.
...
2. **Read files and spawn all autonomous agents in wave simultaneously:**
   ...
   @./.claude/basel-gsd/references/checkpoints.md
   @./.claude/basel-gsd/references/tdd.md
   </execution_context>
...
1. **Spawn agent for checkpoint plan:**
   ```
   Task(prompt="{subagent-task-prompt}", subagent_type="basel-gsd-executor", model="{executor_model}")
   ```
...
6. **Spawn continuation agent (NOT resume):**
   ...
   Task(
     ...
     subagent_type="basel-gsd-executor",
...
Verify what actually exists in the code.",
  subagent_type="basel-gsd-verifier",
...
| `gaps_found` | Present gap summary, offer `/basel:plan-phase {phase} --gaps` |
...
- `/basel:verify-work {X}` — manual testing before planning
```

User runs `/basel:plan-phase {X} --gaps` which:
1. Reads VERIFICATION.md gaps
2. Creates additional plans (04, 05, etc.) with `gap_closure: true` to close gaps
3. User then runs `/basel:execute-phase {X} --gaps-only`
...
1. Run `/basel:execute-phase {phase}` again
