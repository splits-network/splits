...
@./.claude/basel-gsd/references/git-integration.md
</required_reading>
...
2. Use Task tool with subagent_type="basel-gsd-executor" and model="{executor_model}":
...
2. For each segment in order:
   ...
   B. If routing = Subagent:
      ```
      Spawn Task tool with subagent_type="basel-gsd-executor" and model="{executor_model}":
...
See `./.claude/basel-gsd/references/tdd.md` for TDD plan structure.
</tdd_plan_execution>
...
See ./.claude/basel-gsd/references/checkpoints.md for complete checkpoint guidance.
</step>
...
Create `.planning/phases/XX-name/{phase}-USER-SETUP.md` using template from `./.claude/basel-gsd/templates/user-setup.md`.
...
Use ./.claude/basel-gsd/templates/summary.md for structure.
...
**Also available:**
- `/basel:verify-work {phase}-{plan}` — manual acceptance testing before continuing
...
**Also available:**
- `/basel:verify-work {Z}` — manual acceptance testing before continuing
- `/basel:discuss-phase {Z+1}` — gather context first
...
**Also available:**
- `/basel:verify-work` — manual acceptance testing before completing milestone
- `/basel:add-phase <description>` — add another phase before completing
