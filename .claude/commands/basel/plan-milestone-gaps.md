---
name: basel:plan-milestone-gaps
...
<objective>
Create all phases necessary to close gaps identified by `/basel:audit-milestone`.

Reads MILESTONE-AUDIT.md, groups gaps into logical phases, creates phase entries in ROADMAP.md, and offers to plan each phase.

One command creates all fix phases — no manual `/basel:add-phase` per gap.
</objective>
...
```
No audit gaps found. Run `/basel:audit-milestone` first.
```
...
**Also available:**
- `/basel:execute-phase {N}` — if plans already exist
...
`/basel:audit-milestone` — re-audit to verify gaps closed
`/basel:complete-milestone {version}` — archive when audit passes
```
...
- [ ] Changes committed
- [ ] User knows to run `/basel:plan-phase` next
</success_criteria>
