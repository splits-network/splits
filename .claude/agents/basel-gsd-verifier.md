...
When gaps are found, structure them for consumption by `/basel:plan-phase --gaps`.
...
The planner (`/basel:plan-phase --gaps`) reads this gap analysis and creates appropriate plans.
...
## Basel Design Compliance Verification

After standard goal-backward verification (Steps 0-9), perform Basel compliance checks on all files created or modified during this phase.

**These checks are MANDATORY for phases that involve any UI work** (components, pages, styles). Skip these checks ONLY for phases that are purely backend (services, migrations, no frontend files).

### Step B1: Color System Scan

```bash
# Memphis color violations
grep -rn "bg-coral\|text-coral\|border-coral\|bg-teal\|text-teal\|border-teal\|bg-cream\|border-cream\|bg-dark\|bg-purple\|text-purple\|bg-yellow\|text-yellow" --include="*.tsx" --include="*.ts" .

# Raw Tailwind palette violations
grep -rnE "(bg|text|border)-(red|blue|green|yellow|purple|pink|orange|cyan|violet|emerald|lime|amber|rose|sky|indigo|fuchsia|teal|slate|zinc|stone)-[0-9]" --include="*.tsx" .

# Hardcoded hex violations (exclude clipPath and SVG)
grep -rn "style={{" --include="*.tsx" . | grep -i "#[0-9A-Fa-f]\{3,8\}" | grep -v "clipPath"

# bg-white / text-white violations
grep -rn "bg-white\|text-white" --include="*.tsx" .
```

**Result:** Any violation → add to GAPS with `[Basel-Color]` prefix and `severity: critical`.

### Step B2: Typography Scan
...
**For each match:** Assess context. Is this decorative UI chrome or human-readable content?
- Human-readable (labels, descriptions, table cells, timestamps, badges, error messages) → add to GAPS with `[Basel-Typography]` prefix, `severity: warning`
- Decorative chrome → note as acceptable

### Step B3: Rounded Corners Scan

```bash
grep -rnE "rounded-(sm|md|lg|xl|2xl|3xl)" --include="*.tsx" .
```

**Result:** Each match is a violation (exception: `rounded-full`). Add to GAPS with `[Basel-Rounded]` prefix, `severity: critical`.

### Step B4: Gradient Scan

```bash
grep -rn "bg-gradient-to\|linear-gradient\|radial-gradient\|conic-gradient" --include="*.tsx" .
```

**Result:** Each match is a critical violation. Add to GAPS with `[Basel-Gradient]` prefix, `severity: critical`.

### Step B5: DaisyUI Compliance Scan

```bash
# Custom dropdown implementations
grep -rn "click.*outside\|clickOutside\|useClickOutside" --include="*.tsx" .

# Custom modal state (not using <dialog>)
grep -rn "isModalOpen\|showModal\|setShowModal\|isDialogOpen\|setIsDialogOpen" --include="*.tsx" .

# Custom tooltip implementations
grep -rn "onMouseEnter.*tooltip\|tooltip.*onMouse\|setShowTooltip" --include="*.tsx" .
```

**For each match:** Verify whether DaisyUI equivalent was used or a custom implementation was built. Custom implementation → add to GAPS with `[Basel-DaisyUI]` prefix, `severity: warning`.

### Step B6: GSAP Animation Scan

```bash
# Find GSAP usage
grep -rn "gsap\.\(fromTo\|to\)\|gsap\.timeline" --include="*.tsx" .

# Check for clearProps
grep -rn "clearProps" --include="*.tsx" .

# Check for useGSAP vs raw useEffect with gsap
grep -rn "useEffect.*gsap\|useEffect.*ScrollTrigger" --include="*.tsx" .
```

**Cross-reference:** Any animation file using `y:`, `x:`, `rotateX:`, `scale:` without a corresponding `clearProps: "transform"` → add to GAPS with `[Basel-GSAP]` prefix, `severity: warning`.

Raw `useEffect` for GSAP animations instead of `useGSAP` → add to GAPS with `[Basel-GSAP]` prefix, `severity: warning`.

### Step B7: Component Placement Scan

```bash
# Find Basel components in apps — check they're in the right location
find apps -name "*.tsx" -path "*/components/*" | grep -i "basel" | grep -v "/components/basel/"
```

**Verify:** All Basel component files are in one of the two approved locations:
1. `packages/basel-ui/src/` (shared across apps)
2. `apps/{app}/src/components/basel/` (local to one app)

Anything else → add to GAPS with `[Basel-Placement]` prefix, `severity: warning`.

### Step B8: Editorial Layout Check

For each **new page** created during this phase:
- Check for `border-l-4` accent patterns
- Check for kicker text pattern (`uppercase tracking-[0.2em]`)
- Check for responsive split-screen composition (`flex flex-col lg:flex-row` or responsive grid)
- Check for editorial heading hierarchy (`font-black`)

Missing all four patterns on a new page → add to GAPS with `[Basel-Editorial]` prefix, `severity: warning` (not critical — editorial layout is a guideline, not absolute rule for utility pages).

### Basel Gap Format

When recording Basel violations in the VERIFICATION.md, use this format in the gaps YAML:
...
gaps:
  - id: B1
    type: Basel-Color
    severity: critical
    file: "apps/portal/src/app/matches-basel/page.tsx"
    line: 45
    description: "bg-coral used — Memphis color not allowed in Basel"
    fix: "Replace with bg-primary"
  - id: B2
    type: Basel-Typography
    severity: warning
    file: "apps/portal/src/app/matches-basel/components/match-card.tsx"
    line: 23
    description: "text-xs on card description — minimum is text-sm for human-readable content"
    fix: "Change text-xs to text-sm"
```

### Basel Verification Summary

After all B-steps, append a Basel Compliance section to VERIFICATION.md:

```markdown
## Basel Design Compliance

| Check | Status | Violations |
|-------|--------|-----------|
| B1: Color System | PASS/FAIL | N violations |
| B2: Typography | PASS/FAIL | N violations |
| B3: Rounded Corners | PASS/FAIL | N violations |
| B4: Gradients | PASS/FAIL | N violations |
| B5: DaisyUI Compliance | PASS/FAIL | N violations |
| B6: GSAP Animations | PASS/FAIL | N violations |
| B7: Component Placement | PASS/FAIL | N violations |
| B8: Editorial Layout | PASS/FAIL | N violations |

**Basel Score:** X/8 checks passed
```
...
Structured gaps in VERIFICATION.md frontmatter for `/basel:plan-phase --gaps`.
...
**Structure gaps in YAML frontmatter.** The planner (`/basel:plan-phase --gaps`) creates plans from your analysis.
