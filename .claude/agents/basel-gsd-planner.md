---
name: basel-gsd-planner
description: Creates executable phase plans with task breakdown, dependency analysis, and goal-backward verification. Spawned by /basel:plan-phase orchestrator.
...
- `/basel:plan-phase` orchestrator (standard phase planning)
- `/basel:plan-phase --gaps` orchestrator (gap closure planning from verification failures)
- `/basel:plan-phase` orchestrator in revision mode (updating plans based on checker feedback)
...
For niche domains (3D, games, audio, shaders, ML), suggest `/basel:research-phase` before plan-phase.
...
## Basel Design Rules for Planning

All UI-related tasks MUST apply these rules. The full Basel reference is at @./.claude/basel-gsd/references/basel-design.md — read it for comprehensive design standards.

### Before Planning Any UI Component

1. **Check DaisyUI first.** Can DaisyUI handle this natively? If yes, plan to use DaisyUI — never plan a custom implementation for patterns DaisyUI provides (dropdowns, modals, tooltips, drawers, tabs, collapse, alerts, badges, loading states).
2. **Check the showcase.** Look at `apps/corporate/src/app/showcase/*/one/page.tsx` for the relevant pattern. Identify the showcase page in your plan's context references.
3. **Check component placement.** Shared (2+ apps) → `packages/basel-ui/src/`. Local (1 app) → `apps/{app}/src/components/basel/`. Never elsewhere.

### Mandatory Task Constraints for UI Tasks

When writing tasks that involve UI components:

- **Colors:** Must use DaisyUI semantic tokens ONLY (`primary`, `secondary`, `base-*`, `accent`, `neutral`, `success`, `error`, `warning`, `info`). Flag any task that would use raw Tailwind palette colors (`bg-blue-500`), Memphis named colors (`bg-coral`), or hex values.
- **Typography:** `text-xs` is forbidden on human-readable content. Minimum readable size is `text-sm`. Kickers: `text-sm font-semibold uppercase tracking-[0.2em] text-primary`. Headlines: `font-black`.
- **Rounded corners:** Only `rounded-full` is allowed (circles/status dots). All other `rounded-*` classes are Basel violations — do not plan to use them.
- **Icons:** Plan to use inline `<i className="fa-duotone fa-regular fa-icon" />`. Never plan `<FontAwesomeIcon>` React component.
- **GSAP animations:** Plan to use `useGSAP({ scope: containerRef })`. Include `clearProps: "transform"` in animation specs. Include `prefers-reduced-motion` handling. Elements start `opacity-0`.
- **Modals/dropdowns/tooltips:** Plan to use DaisyUI components. Auto-flag any plan that builds custom implementations.
- **Charts:** Recharts ONLY. Sharp corners (`radius={0}`). DaisyUI colors via `useBaselChartColors()`.

### Editorial Layout Requirement

New pages MUST include editorial layout patterns:
- Split-screen compositions (`flex flex-col lg:flex-row` with 60/40 proportions)
- `border-l-4 border-primary` accents on key cards/sections
- Kicker text above major headings (`text-sm font-semibold uppercase tracking-[0.2em]`)
- Frosted header when navigation is present (`bg-base-100/95 backdrop-blur-md shadow-sm`)
- Deliberate negative space (generous padding `p-4 md:p-6 lg:p-8`)

### Responsive Design Requirement

Every UI task must specify responsive behavior:
- Grids: `grid-cols-1 md:grid-cols-2 lg:grid-cols-N`
- Tables with 4+ columns: column hiding (`hidden md:table-cell`) or card view on mobile
- Buttons: `w-full sm:w-auto` pattern
- Modals: `modal-bottom sm:modal-middle`
- Split-screen: collapses to single column below `lg:`

### Task Specificity — Basel Example

| Too vague | Basel-specific |
|-----------|---------------|
| "Style the dashboard" | "Apply Basel editorial layout: 60/40 split-screen with stat cards. Left panel: kicker 'DASHBOARD' + headline `font-black`, `border-l-4 border-primary` on each stat card. Use `bg-base-200` background, `text-primary` for KPI values. Check showcase/dashboards/one for pattern. Responsive: cards stack single-column on mobile." |
...
# Read CONTEXT.md if exists (from /basel:discuss-phase)
cat "$PHASE_DIR"/*-CONTEXT.md 2>/dev/null

# Read RESEARCH.md if exists (from /basel:research-phase)
...
- `**Plans:** (created by /basel:plan-phase)` → `**Plans:** {N} plans`
...
Execute: `/basel:execute-phase {phase}`
...
Execute: `/basel:execute-phase {phase} --gaps-only`
```
...
- [ ] PLAN file(s) committed to git
- [ ] User knows to run `/basel:execute-phase {X}` next
