...
You are spawned by `/basel:execute-phase` orchestrator.
...
<!-- Basel Design Deviation Rules -->

**RULE 5: Auto-fix DaisyUI violations**

**Trigger:** Custom implementation found where DaisyUI pattern exists — custom dropdown with click-outside listener, custom modal with isOpen state, custom tooltip with hover state, custom accordion with toggle state, etc.

**Action:** Replace with DaisyUI equivalent immediately. No user permission needed.

**Examples:**
- Custom `isOpen` state + click-outside → `<div class="dropdown">`
- Custom dialog state + overlay → `<dialog class="modal">`
- `data-tooltip` custom attribute → `class="tooltip" data-tip="..."`
- Custom collapse toggle → `<div class="collapse">`

Track as deviation: `[Rule 5 - DaisyUI] [description of what was replaced]`

---

**RULE 6: Auto-fix Basel color violations**

**Trigger:** Memphis named color, raw Tailwind palette color, or hardcoded hex found in files being created or modified.

**Action:** Replace with correct DaisyUI semantic token immediately.
...
bg-white   → bg-base-100         text-white → text-base-100-content or appropriate -content
```

Track as deviation: `[Rule 6 - Color] [original class → replacement]`

---

**RULE 7: Auto-fix rounded corners**

**Trigger:** Any `rounded-sm`, `rounded`, `rounded-md`, `rounded-lg`, `rounded-xl`, `rounded-2xl`, `rounded-3xl` on structural elements (cards, buttons, inputs, containers, badges, panels).

**Exception:** `rounded-full` on avatars and status indicators is ALLOWED.

**Action:** Remove the class immediately. Basel uses sharp corners.

Track as deviation: `[Rule 7 - Rounded] [removed class and element]`

---

**RULE 8: Auto-add GSAP clearProps**

**Trigger:** GSAP `fromTo()` or `to()` animation that uses `y`, `x`, `rotateX`, `rotateY`, `scale`, `skewX`, or `skewY` — WITHOUT `clearProps: "transform"`.

**Action:** Add `clearProps: "transform"` to the animation config immediately.

**Why:** GSAP leaves residual `transform` on elements even when animating to `0`. This creates a CSS containing block that traps `position: fixed` children inside it, breaking DaisyUI modals, dropdowns, and other fixed-position elements.

Track as deviation: `[Rule 8 - GSAP] [added clearProps to animation in file]`

---

**RULE 9: Auto-fix text-xs on human-readable content**

**Trigger:** `text-xs` class on any element containing human-readable text (labels, descriptions, table cells, error messages, tooltips, helper text, captions, timestamps, badges).

**Exception:** Decorative UI chrome that users do not need to read.

**Action:** Replace `text-xs` with `text-sm`.

Track as deviation: `[Rule 9 - Typography] [changed text-xs to text-sm in file:line]`

---

**RULE 10: Check showcase before implementing new UI patterns**

**Trigger:** Plan task requires building a new page layout or major UI pattern not previously seen in this execution.

**Action:** Before implementing, read the relevant showcase page from `apps/corporate/src/app/showcase/*/one/page.tsx`. Use it as a design reference. Consult @./.claude/basel-gsd/references/basel-design.md section 7 for the showcase directory map.

This is a standard step, not a deviation. Do not track in the deviation log.
...
**See @./.claude/basel-gsd/references/checkpoints.md**
...
**Use template from:** @./.claude/basel-gsd/templates/summary.md
