    ...
    return (
        <div className={['flex flex-wrap items-center justify-between gap-4', className].filter(Boolean).join(' ')}>
            ...
            <div className="flex items-center gap-3">
                ...
                <h2
                    className="text-2xl font-black uppercase tracking-tight text-dark"
                >
                    {month} {year}
                </h2>
                <button
                ...
                <div className="flex gap-0 border-lg">
                    {viewModes.map((v, i) => (
                        <button
                            ...
                            className={[
                                'px-5 py-2 font-black text-xs uppercase tracking-wider transition-colors',
                                activeViewMode === v ? 'bg-dark text-yellow' : 'bg-white text-dark',
                                i < viewModes.length - 1 ? 'border-r-2 border-r-dark' : '',
                            ].filter(Boolean).join(' ')}
                        >
                            {v}
                        </button>
