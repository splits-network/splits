...
export function SupportCTA({
    ...
    return (
        <section
            className={['p-8 text-center', className].filter(Boolean).join(' ')}
            style={{ background: COLORS.dark, border: `4px solid ${COLORS.teal}` }}
        >
            <i className={`fa-duotone fa-solid ${icon} text-5xl mb-4`} style={{ color: COLORS.teal }} />
            ...
            {actions.length > 0 && (
                <div className="flex flex-wrap gap-4 justify-center mt-6">
                    {actions.map((action) => {
                        const hex = ACCENT_HEX[action.color || 'teal'];
                        if (action.href) {
                            return (
                                <a
                                    key={action.label}
                                    ...
                                    style={{ background: hex, border: `4px solid ${hex}`, color: COLORS.dark }}
                                >
