...
import { expandApply } from "./expandApply.js"
...
export const generatePlugins = async ({ type, srcDir, distDir, exclude = [] }) => {
    ...
    await Promise.all(
        filteredCssFiles.map(async (cssFile) => {
            const { content: cssContent } = await cssToJs(`${srcDir}/${cssFile}.css`)
            // Expand @apply directives to raw CSS before objectification
            const expandedCss = await expandApply(cssContent)
            const componentDir = await createDirectoryBasedOnFileNames(cssFile, ".css", distDir)
            await createPluginFiles(type, componentDir, expandedCss, cssFile)
        }),
    )
}
