...
export class ArtifactService {
  ...
  async getEntityLinks(callId: string, params: ArtifactListParams, clerkUserId: string) {
    await this.accessResolver.resolve(clerkUserId);
    const { data, total } = await this.repository.findEntityLinks(callId, params);
  ...
  async createEntityLink(callId: string, input: AddEntityLinkInput, clerkUserId: string) {
    await this.accessResolver.resolve(clerkUserId);
    const link = await this.repository.createEntityLink(callId, input);

    await this.eventPublisher?.publish('call.entity_linked', {
      call_id: callId,
      link_id: link.id,
  ...
  async deleteEntityLink(callId: string, id: string, clerkUserId: string) {
    await this.accessResolver.resolve(clerkUserId);
