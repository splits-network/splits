...
import {
  AddParticipantInput, ParticipantListParams,
  callIdParamSchema, participantIdParamSchema,
  listQuerySchema, addParticipantSchema,
} from './types';
...
export function registerParticipantRoutes(
  ...
  const service = new ParticipantService(repository, supabase, eventPublisher);

  // GET /api/v3/calls/:callId/participants
  app.get('/api/v3/calls/:callId/participants', {
  ...
  app.get('/api/v3/calls/:callId/participants/:id', {
  ...
  app.post('/api/v3/calls/:callId/participants', {
  ...
  app.delete('/api/v3/calls/:callId/participants/:id', {
    schema: { params: participantIdParamSchema },
  }, async (request, reply) => {
    const clerkUserId = request.headers['x-clerk-user-id'] as string;
    if (!clerkUserId) {
