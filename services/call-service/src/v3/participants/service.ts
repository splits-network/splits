...
export class ParticipantService {
  ...
  async getAll(callId: string, params: ParticipantListParams, clerkUserId: string) {
    await this.accessResolver.resolve(clerkUserId);
    const { data, total } = await this.repository.findAllForCall(callId, params);
  ...
  async create(callId: string, input: AddParticipantInput, clerkUserId: string) {
    await this.accessResolver.resolve(clerkUserId);

    const resolvedUserId = await this.repository.resolveUserId(input.user_id);
    if (!resolvedUserId) throw new BadRequestError('Could not resolve participant user');
    ...
    await this.eventPublisher?.publish('call.participant_added', {
      call_id: callId,
  ...
  async delete(callId: string, id: string, clerkUserId: string) {
    await this.accessResolver.resolve(clerkUserId);
    ...
    await this.eventPublisher?.publish('call.participant_removed', {
      call_id: callId,
      participant_id: id,
    }, 'call-service');
  }
