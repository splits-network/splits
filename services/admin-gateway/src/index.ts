...
async function main() {
    ...
    const adminClerkSecretKey = baseConfig.nodeEnv === 'production'
        ? getEnvOrThrow('ADMIN_CLERK_SECRET_KEY')
        : (process.env.ADMIN_CLERK_SECRET_KEY ?? '');

    if (!adminClerkSecretKey) {
        logger.warn('ADMIN_CLERK_SECRET_KEY not set — auth will fail for all requests');
    }

    const supabaseServiceRoleKey = dbConfig.supabaseServiceRoleKey;
    if (!supabaseServiceRoleKey) {
        throw new Error('SUPABASE_SERVICE_ROLE_KEY is required for admin-gateway');
    }
    ...
    const services: Record<string, string> = {
        ...
        automation: process.env.AUTOMATION_SERVICE_URL || 'http://localhost:3007',
        'document-processing': process.env.DOCUMENT_PROCESSING_SERVICE_URL || 'http://localhost:3008',
        ...
        integration: process.env.INTEGRATION_SERVICE_URL || 'http://localhost:3016',
