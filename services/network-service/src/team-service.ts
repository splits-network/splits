    ...
    switch (model) {
      ...
      case 'tiered_split':
        if (typeof config.owner_percentage !== 'number' || config.owner_percentage <= 0 || config.owner_percentage >= 100) {
          throw new Error('Tiered split requires valid owner_percentage (0-100)');
        }
        break;
      ...
      case 'hybrid':
        if (typeof config.team_overhead_fee !== 'number' || config.team_overhead_fee < 0 || config.team_overhead_fee >= 100) {
          throw new Error('Hybrid split requires valid team_overhead_fee (0-100)');
        }
        break;
      default:
  ...
  private applySplitModel(
    ...
    switch (model) {
      ...
      case 'tiered_split': {
        ...
        if (!owner) {
          throw new Error('Team must have an owner for tiered split');
        }
      ...
      case 'individual_credit': {
        // In individual credit, typically the primary recruiter gets 100%
        // For team context, this might mean the submitter gets it all
        // This requires context about who submitted - for now, split equally
        const splitPercentage = 100 / members.length;
        const splitAmount = placementFee / members.length;
        members.forEach((member) => {
          splits.push({
      ...
      case 'hybrid': {
        const teamOverheadFee = config.team_overhead_fee;
        const teamOverheadAmount = (placementFee * teamOverheadFee) / 100;
        
        // Team overhead goes to owner
        const owner = members.find((m) => m.role === 'owner');
        if (!owner) {
          throw new Error('Team must have an owner for hybrid split');
        }

        const remainingAmount = placementFee - teamOverheadAmount;
        const remainingPercentage = 100 - teamOverheadFee;
        const memberSplitPercentage = remainingPercentage / members.length;
        const memberSplitAmount = remainingAmount / members.length;
        ...
        // Add overhead to owner's split
        const ownerSplit = splits.find((s) => s.recruiter_id === owner.recruiter_id);
        if (ownerSplit) {
          ownerSplit.percentage += teamOverheadFee;
          ownerSplit.amount += teamOverheadAmount;
          ownerSplit.notes = 'Includes team overhead';
        }
        break;
      }
    }
