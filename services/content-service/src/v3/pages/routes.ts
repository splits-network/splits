...
export function registerPageRoutes(
  ...
  app.get('/api/v3/pages/views/by-slug/:slug', {
  ...
  app.get('/api/v3/pages', {
    schema: { querystring: listQuerySchema },
  }, async (request, reply) => {
    const result = await service.getAll(request.query as PageListParams);
    return reply.send({ data: result.data, pagination: result.pagination });
  });

  // GET /api/v3/pages/:id
  app.get('/api/v3/pages/:id', {
