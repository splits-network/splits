...
async function main() {
    const baseConfig = loadBaseConfig("onboarding-service");
    ...
    await app.register(swagger as any, {
        openapi: {
            info: {
                ...
                description:
                    "Handles all onboarding flows: business, recruiter, candidate",
                version: "1.0.0",
            },
            servers: [
                {
            ...
            tags: [
                { name: "business", description: "Business/company onboarding" },
            ],
        },
    });
    ...
    try {
        ...
        logger.info(`Onboarding service listening on port ${baseConfig.port}`);
    } catch (err) {
        logger.error(err);
        if (process.env.SENTRY_DSN) {
