/**
 * Onboarding V3 Routes — API Gateway
 *
...
export function registerOnboardingV3Routes(
  app: FastifyInstance,
  ...
  // ── GET /api/v3/onboarding/status ────────────────────────────────────
  app.get(
    '/api/v3/onboarding/status',
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      const [userResult, candidateResult, recruiterResult] = await Promise.allSettled([
        identityService().get('/api/v3/users/me', undefined, correlationId, authHeaders),
        atsService().get('/api/v3/candidates/me', undefined, correlationId, authHeaders),
        networkService().get('/api/v3/recruiters/me', undefined, correlationId, authHeaders),
      ]);
  ...
  // ── POST /api/v3/onboarding/init ─────────────────────────────────────
  app.post(
    '/api/v3/onboarding/init',
    { preHandler: requireAuth() },
    async (request: FastifyRequest, reply: FastifyReply) => {
      const correlationId = getCorrelationId(request);
      const authHeaders = buildAuthHeaders(request);
        ...
        if (body.source_app === 'candidate') {
          try {
            const candidateResponse = await atsService().get<any>(
              '/api/v3/candidates/me', undefined, correlationId, authHeaders
            );
  ...
  // ── POST /api/v3/onboarding/candidate ────────────────────────────────
  app.post(
    '/api/v3/onboarding/candidate',
    { preHandler: requireAuth() },
    async (request: FastifyRequest, reply: FastifyReply) => {
      const correlationId = getCorrelationId(request);
      const authHeaders = buildAuthHeaders(request);
      ...
      try {
        ...
        if (body.profile && Object.keys(body.profile).length > 0) {
          updates.push(
            atsService().patch(
              `/api/v3/candidates/${body.candidate_id}`,
              body.profile,
              correlationId,
              authHeaders
            )
  ...
  // ── POST /api/v3/onboarding/recruiter ────────────────────────────────
  app.post(
    '/api/v3/onboarding/recruiter',
    { preHandler: requireAuth() },
    async (request: FastifyRequest, reply: FastifyReply) => {
      const correlationId = getCorrelationId(request);
      const authHeaders = buildAuthHeaders(request);
