...
export async function registerChatRoutes(app: FastifyInstance, config: RegisterChatRoutesConfig) {
    ...
    app.post('/api/v2/chat/conversations', async (request, reply) => {
        try {
            ...
            const conversation = await service.createOrFindConversation(clerkUserId, {
    ...
    app.get('/api/v2/chat/conversations/:id/messages', async (request, reply) => {
        try {
            const { clerkUserId } = requireUserContext(request);
    ...
    app.post('/api/v2/chat/conversations/:id/messages', async (request, reply) => {
        try {
            const { clerkUserId } = requireUserContext(request);
            const { id } = request.params as { id: string };
            const body = request.body as any;
    ...
    app.post('/api/v2/chat/conversations/:id/accept', async (request, reply) => {
        try {
            const { clerkUserId } = requireUserContext(request);
            const { id } = request.params as { id: string };
    ...
    app.post('/api/v2/chat/conversations/:id/decline', async (request, reply) => {
        try {
            const { clerkUserId } = requireUserContext(request);
            const { id } = request.params as { id: string };
    ...
    app.delete('/api/v2/chat/conversations/:id/mute', async (request, reply) => {
        try {
            const { clerkUserId } = requireUserContext(request);
            const { id } = request.params as { id: string };
    ...
    app.post('/api/v2/chat/conversations/:id/archive', async (request, reply) => {
        try {
            const { clerkUserId } = requireUserContext(request);
            const { id } = request.params as { id: string };
    ...
    app.delete('/api/v2/chat/conversations/:id/archive', async (request, reply) => {
        try {
            const { clerkUserId } = requireUserContext(request);
            const { id } = request.params as { id: string };
    ...
    app.post('/api/v2/chat/conversations/:id/read-receipt', async (request, reply) => {
        try {
            const { clerkUserId } = requireUserContext(request);
            const { id } = request.params as { id: string };
            const body = request.body as any;
            await service.updateReadReceipt(clerkUserId, id, body.lastReadMessageId);
            return reply.code(204).send();
        } catch (error: any) {
            return reply.code(error.statusCode || 400).send({ error: error.message });
        }
    ...
    app.post('/api/v2/chat/blocks', async (request, reply) => {
        try {
            const { clerkUserId } = requireUserContext(request);
            const body = request.body as any;
            await service.blockUser(clerkUserId, body.blockedUserId, body.reason);
            return reply.code(204).send();
        } catch (error: any) {
            return reply.code(error.statusCode || 400).send({ error: error.message });
        }
    ...
    app.post('/api/v2/chat/attachments/init', async (request, reply) => {
        try {
            if (!attachmentsEnabled) {
                return reply.code(403).send({ error: 'Attachments are disabled' });
            }
            const { clerkUserId } = requireUserContext(request);
            const body = request.body as any;
            ...
            const attachment = await service.createAttachment(clerkUserId, body.conversationId, {
                file_name: fileName,
    ...
    app.post('/api/v2/chat/attachments/:id/complete', async (request, reply) => {
        try {
            if (!attachmentsEnabled) {
                return reply.code(403).send({ error: 'Attachments are disabled' });
            }
            const { clerkUserId } = requireUserContext(request);
            const { id } = request.params as { id: string };
            const attachment = await service.markAttachmentUploaded(clerkUserId, id);
    ...
    app.get('/api/v2/chat/attachments/:id/download-url', async (request, reply) => {
        try {
            if (!attachmentsEnabled) {
                return reply.code(403).send({ error: 'Attachments are disabled' });
            }
    ...
    app.patch('/api/v2/chat/messages/:id', async (request, reply) => {
        try {
            ...
            const { id } = request.params as { id: string };
            const body = request.body as any;
            const message = await service.updateMessageRedaction(clerkUserId, id, {
    ...
    app.get('/api/v2/admin/chat/reports', async (request, reply) => {
        try {
            ...
            const limit = Math.min(parseInt(query.limit || '25', 10), 100);
            const cursor = query.cursor as string | undefined;
            const result = await service.listReports(clerkUserId, limit, cursor);
            return reply.send({
                data: result.data,
                pagination: {
                    total: result.total,
    ...
    app.get('/api/v2/admin/chat/reports/:id/evidence', async (request, reply) => {
        try {
            const { clerkUserId } = requireUserContext(request);
            const { id } = request.params as { id: string };
    ...
    app.post('/api/v2/admin/chat/reports/:id/action', async (request, reply) => {
        try {
            const { clerkUserId } = requireUserContext(request);
            const { id } = request.params as { id: string };
            const body = request.body as any;
    ...
    app.get('/api/v2/admin/chat/audit', async (request, reply) => {
        try {
            ...
            const limit = Math.min(parseInt(query.limit || '25', 10), 100);
            const cursor = query.cursor as string | undefined;
            const result = await service.listModerationAudit(clerkUserId, limit, cursor);
            return reply.send({
                data: result.data,
                pagination: {
                    total: result.total,
    ...
    app.get('/api/v2/admin/chat/metrics', async (request, reply) => {
        try {
            ...
            const result = await service.getAdminMetrics(clerkUserId, rangeDays);
            return reply.send({ data: result });
        } catch (error: any) {
            return reply.code(error.statusCode || 400).send({ error: error.message });
        }
