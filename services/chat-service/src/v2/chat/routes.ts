...
export async function registerChatRoutes(
    ...
    // V3 alias — same handler, V3 path for frontend migration
    app.get("/api/v3/chat/presence", async (request, reply) => {
        try {
            requireUserContext(request);
            const query = request.query as any;
            const rawUserIds = query.userIds;
    ...
    // ── V3 alias — same createOrFindConversation with context + routing ──
    ...
    // ── V3 alias — same list with participant data ──
    app.get("/api/v3/chat/conversations", async (request, reply) => {
        try {
            const { clerkUserId } = requireUserContext(request);
            const query = request.query as any;
    ...
    app.post(
        "/api/v3/chat/conversations/:id/decline",
        async (request, reply) => {
            try {
                const { clerkUserId } = requireUserContext(request);
                const { id } = request.params as { id: string };
    ...
    app.post("/api/v3/chat/conversations/:id/mute", async (request, reply) => {
        try {
            const { clerkUserId } = requireUserContext(request);
            const { id } = request.params as { id: string };
            await service.muteConversation(clerkUserId, id, true);
    ...
    app.delete(
        "/api/v3/chat/conversations/:id/mute",
        async (request, reply) => {
            try {
                const { clerkUserId } = requireUserContext(request);
                const { id } = request.params as { id: string };
    ...
    app.post(
        "/api/v3/chat/conversations/:id/archive",
        async (request, reply) => {
            try {
                const { clerkUserId } = requireUserContext(request);
                const { id } = request.params as { id: string };
    ...
    app.post(
        "/api/v3/chat/conversations/:id/read-receipt",
        async (request, reply) => {
            try {
                const { clerkUserId } = requireUserContext(request);
                const { id } = request.params as { id: string };
    ...
    app.post("/api/v3/chat/blocks", async (request, reply) => {
        try {
            const { clerkUserId } = requireUserContext(request);
            const body = request.body as any;
            await service.blockUser(
    ...
    app.delete("/api/v3/chat/blocks/:blockedUserId", async (request, reply) => {
        try {
            const { clerkUserId } = requireUserContext(request);
            const { blockedUserId } = request.params as {
                blockedUserId: string;
    ...
    app.post("/api/v3/chat/attachments/init", async (request, reply) => {
        try {
            if (!attachmentsEnabled) {
                return reply
                    .code(403)
    ...
    app.post(
        "/api/v3/chat/attachments/:id/complete",
        async (request, reply) => {
            try {
                if (!attachmentsEnabled) {
                    return reply
    ...
    app.get(
        "/api/v3/chat/attachments/:id/download-url",
        async (request, reply) => {
            try {
                if (!attachmentsEnabled) {
                    return reply
    ...
    app.patch("/api/v3/chat/messages/:id", async (request, reply) => {
        try {
            const { clerkUserId } = requireUserContext(request);
            const { id } = request.params as { id: string };
            const body = request.body as any;
