...
import {
  CreateATSInput, UpdateATSInput, ATSListParams,
  idParamSchema, listQuerySchema, createATSSchema, updateATSSchema,
} from './types';
...
export function registerATSIntegrationRoutes(
  ...
  const service = new ATSIntegrationService(repository, supabase, eventPublisher);

  // GET /api/v3/integrations/ats
  app.get('/api/v3/integrations/ats', {
  ...
  app.get('/api/v3/integrations/ats/:id', {
  ...
  app.post('/api/v3/integrations/ats', {
  ...
  app.patch('/api/v3/integrations/ats/:id', {
  ...
  app.delete('/api/v3/integrations/ats/:id', {
    schema: { params: idParamSchema },
  }, async (request, reply) => {
    const clerkUserId = request.headers['x-clerk-user-id'] as string;
    if (!clerkUserId) {
