...
export function registerConnectionRoutes(
  ...
  const oauthService = new OAuthService(repository, providerRepo, eventPublisher, logger, crypto);
  ...
  app.post('/api/v3/integrations/connections/initiate', {
    ...
    try {
      const result = await oauthService.initiateOAuth(
        clerkUserId, body.provider_slug, body.redirect_uri, body.organization_id,
      );
      return reply.send({ data: result });
    } catch (err: any) {
      return reply.status(400).send({ error: { code: 'INITIATE_FAILED', message: err.message } });
    }
  });

  // POST /api/v3/integrations/connections/callback — handle OAuth callback
  app.post('/api/v3/integrations/connections/callback', {
    schema: { body: callbackSchema },
  }, async (request, reply) => {
    const body = request.body as CallbackInput;

    try {
      const connection = await oauthService.handleCallback(body.code, body.state);
      return reply.send({ data: connection });
    } catch (err: any) {
      return reply.status(400).send({ error: { code: 'CALLBACK_FAILED', message: err.message } });
    }
  });
}
