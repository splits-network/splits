...
import { GmailClient, GmailMessage } from './gmail-client.js';
...
export class EmailService {
    ...
    async sendMessage(
        ...
        if (connection.provider_slug.startsWith('google_')) {
            ...
            await this.connectionAdapter.update(connectionId, {
    ...
    private async authorize(connectionId: string, clerkUserId: string) {
        const connection = await this.connectionAdapter.findById(connectionId);
