...
export class GmailClient {
    ...
    /**
     * Trash a message (moves to Trash, recoverable for 30 days).
     */
    async trashMessage(accessToken: string, messageId: string): Promise<void> {
        const res = await fetch(`${BASE_URL}/users/me/messages/${messageId}/trash`, {
            method: 'POST',
            headers: { Authorization: `Bearer ${accessToken}` },
        });
        if (!res.ok) {
            const text = await res.text();
            throw new Error(`Gmail trash failed (${res.status}): ${text}`);
        }
    }
    ...
    async untrashMessage(accessToken: string, messageId: string): Promise<void> {
        const res = await fetch(`${BASE_URL}/users/me/messages/${messageId}/untrash`, {
            method: 'POST',
            headers: { Authorization: `Bearer ${accessToken}` },
        });
        if (!res.ok) {
            const text = await res.text();
            throw new Error(`Gmail untrash failed (${res.status}): ${text}`);
        }
    }

    /**
     * Modify labels on a message (add/remove).
     * Archive = remove INBOX label. Mark read = remove UNREAD label.
     */
    async modifyLabels(
        accessToken: string,
        messageId: string,
        ...
        const res = await fetch(`${BASE_URL}/users/me/messages/${messageId}/modify`, {
            method: 'POST',
            headers: {
                Authorization: `Bearer ${accessToken}`,
                'Content-Type': 'application/json',
        ...
        if (!res.ok) {
            const text = await res.text();
            throw new Error(`Gmail modifyLabels failed (${res.status}): ${text}`);
        }
    }
    ...
    async listLabels(accessToken: string): Promise<GmailLabel[]> {
        const res = await this.request(accessToken, '/users/me/labels');
        return res.labels ?? [];
    }
