...
export interface AdminAtsChartDataResult {
...
export async function getAtsAdminChartData(
    supabase: SupabaseClient,
    period: string
): Promise<AdminAtsChartDataResult> {
    ...
    const buckets = Array.from({ length: config.buckets }, (_, i) => {
        ...
        return { start: start.toISOString(), end: end.toISOString(), label: formatBucketLabel(start, period) };
    });
    ...
    const [jobPostings, applicationVolume, interviewCounts, hireCounts] = await Promise.all([
        Promise.all(
            buckets.map(async b => ({
                label: b.label,
                value: await countInRange(supabase, 'jobs', b.start, b.end),
            }))
        ),
        Promise.all(
            buckets.map(async b => ({
                x: b.label,
                y: await countInRange(supabase, 'applications', b.start, b.end),
            }))
        ),
        Promise.all(
            buckets.map(b =>
                countInRange(supabase, 'applications', b.start, b.end, ['interview', 'offer', 'hired'])
            )
        ),
        Promise.all(
            buckets.map(b =>
                countInRange(supabase, 'applications', b.start, b.end, ['hired'])
            )
        ),
    ]);
