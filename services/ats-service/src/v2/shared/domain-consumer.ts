...
export class DomainEventConsumer {
    ...
    async connect(): Promise<void> {
        if (this.isConnecting) {
            this.logger.debug('V2 ATS consumer connection attempt already in progress, skipping');
            return;
        }
        ...
        try {
            ...
            this.connection!.on('error', (err) => {
                this.logger.error({ err }, 'V2 ATS consumer RabbitMQ connection error');
                this.connectionHealthy = false;
                this.scheduleReconnect();
            });

            this.connection!.on('close', () => {
                this.logger.warn('V2 ATS consumer RabbitMQ connection closed');
                this.connectionHealthy = false;
                if (!this.isClosing) this.scheduleReconnect();
            });

            this.channel.on('error', (err) => {
                this.logger.error({ err }, 'V2 ATS consumer RabbitMQ channel error');
                this.connectionHealthy = false;
                if (!this.isClosing) this.scheduleReconnect();
            });

            this.channel.on('close', () => {
                this.logger.warn('V2 ATS consumer RabbitMQ channel closed');
                this.connectionHealthy = false;
                if (!this.isClosing) this.scheduleReconnect();
            });
    ...
    private scheduleReconnect(): void {
        ...
        if (this.reconnectAttempts > this.maxReconnectAttempts) {
            this.logger.error('V2 ATS consumer max reconnection attempts reached, giving up');
            return;
        }

        const delay = Math.min(1000 * Math.pow(2, this.reconnectAttempts - 1), 30000);
        this.logger.info({ attempt: this.reconnectAttempts, delay }, 'Scheduling V2 ATS consumer reconnection');
        ...
        this.reconnectTimeout = setTimeout(async () => {
            ...
            try {
                ...
                this.logger.error({ err: error }, 'V2 ATS consumer reconnection attempt failed');
            }
        }, delay);
    }
