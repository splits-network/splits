/**
 * Smart Resume Tasks V3 Service -- Business Logic
 *
 * Candidates own their tasks via profile ownership. Recruiters and admins can read.
 */
...
import { CreateSmartResumeTaskInput, UpdateSmartResumeTaskInput, SmartResumeTaskListParams } from './types.js';

export class SmartResumeTaskService {
  private accessResolver: AccessContextResolver;

  constructor(
    private repository: SmartResumeTaskRepository,
  ...
  async getAll(params: SmartResumeTaskListParams, clerkUserId: string) {
  ...
  async getById(id: string, clerkUserId: string) {
    ...
    if (!task) throw new NotFoundError('SmartResumeTask', id);
    return task;
  }

  async create(input: CreateSmartResumeTaskInput, clerkUserId: string) {
    ...
    if (input.impact !== undefined) record.impact = input.impact;
  ...
  async update(id: string, input: UpdateSmartResumeTaskInput, clerkUserId: string) {
    const existing = await this.repository.findById(id);
    if (!existing) throw new NotFoundError('SmartResumeTask', id);
    ...
    if (!updated) throw new NotFoundError('SmartResumeTask', id);
