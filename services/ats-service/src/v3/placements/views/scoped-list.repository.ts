/**
 * Placements Scoped List View Repository
 *
 * Applies role-based scoping (candidate, recruiter, organization)
 * to placement listings. The organization scope requires a join
 * through jobs -> companies to resolve org membership.
 */
...
export class ScopedPlacementListRepository {
  ...
  async findAll(
    ...
    if (scopeFilters.organization_ids && scopeFilters.organization_ids.length > 0) {
      const jobIds = await this.resolveOrgJobIds(scopeFilters.organization_ids);
  ...
  private async resolveOrgJobIds(organizationIds: string[]): Promise<string[]> {
    const { data: orgJobs } = await this.supabase
      .from('jobs')
      .select('id, company:companies!inner(identity_organization_id)')
      .in('company.identity_organization_id', organizationIds);
    return (orgJobs || []).map((j: any) => j.id);
  }
