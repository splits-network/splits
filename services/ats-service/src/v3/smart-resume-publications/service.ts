/**
 * Smart Resume Publications V3 Service — Business Logic
 *
 * Candidates own their publications via profile ownership.
...
export class SmartResumePublicationService {
  private accessResolver: AccessContextResolver;

  constructor(
    private repository: SmartResumePublicationRepository,
  ...
  async getAll(params: SmartResumePublicationListParams, clerkUserId: string) {
  ...
  async getById(id: string, clerkUserId: string) {
    ...
    if (!publication) throw new NotFoundError('SmartResumePublication', id);
    return publication;
  }

  async create(input: CreateSmartResumePublicationInput, clerkUserId: string) {
    ...
    if (input.publication_type !== undefined) record.publication_type = input.publication_type;
    if (input.publisher !== undefined) record.publisher = input.publisher;
    if (input.url !== undefined) record.url = input.url;
    if (input.published_date !== undefined) record.published_date = input.published_date;
  ...
  async update(id: string, input: UpdateSmartResumePublicationInput, clerkUserId: string) {
    const existing = await this.repository.findById(id);
    if (!existing) throw new NotFoundError('SmartResumePublication', id);

    await this.assertOwnership(clerkUserId, existing.profile_id);

    const updates: Record<string, any> = {};
    if (input.title !== undefined) updates.title = input.title;
    if (input.publication_type !== undefined) updates.publication_type = input.publication_type;
    if (input.publisher !== undefined) updates.publisher = input.publisher;
    if (input.url !== undefined) updates.url = input.url;
    if (input.published_date !== undefined) updates.published_date = input.published_date;
    ...
    if (!updated) throw new NotFoundError('SmartResumePublication', id);
