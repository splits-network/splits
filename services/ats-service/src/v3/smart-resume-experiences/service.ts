/**
 * Smart Resume Experiences V3 Service -- Business Logic
 *
 * Ownership is asserted via the parent smart_resume_profiles table.
 * Candidates own their experiences. Recruiters and admins can read.
 */
...
export class SmartResumeExperienceService {
  private accessResolver: AccessContextResolver;

  constructor(
    private repository: SmartResumeExperienceRepository,
  ...
  async getAll(params: SmartResumeExperienceListParams, clerkUserId: string) {
  ...
  async getById(id: string, clerkUserId: string) {
    ...
    if (!experience) throw new NotFoundError('SmartResumeExperience', id);
    return experience;
  }

  async create(input: CreateSmartResumeExperienceInput, clerkUserId: string) {
    ...
    if (input.is_current !== undefined) record.is_current = input.is_current;
    if (input.description !== undefined) record.description = input.description;
    if (input.achievements !== undefined) record.achievements = input.achievements;
  ...
  async update(id: string, input: UpdateSmartResumeExperienceInput, clerkUserId: string) {
    const existing = await this.repository.findById(id);
    if (!existing) throw new NotFoundError('SmartResumeExperience', id);
    ...
    if (input.achievements !== undefined) updates.achievements = input.achievements;
    if (input.visible_to_matching !== undefined) updates.visible_to_matching = input.visible_to_matching;
    if (input.sort_order !== undefined) updates.sort_order = input.sort_order;

    const updated = await this.repository.update(id, updates);
    if (!updated) throw new NotFoundError('SmartResumeExperience', id);
