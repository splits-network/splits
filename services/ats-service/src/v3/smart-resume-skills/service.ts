/**
 * Smart Resume Skills V3 Service — Business Logic
 *
 * Candidates own their skills via profile ownership.
...
export class SmartResumeSkillService {
  private accessResolver: AccessContextResolver;

  constructor(
    private repository: SmartResumeSkillRepository,
  ...
  async getAll(params: SmartResumeSkillListParams, clerkUserId: string) {
  ...
  async getById(id: string, clerkUserId: string) {
    ...
    if (!skill) throw new NotFoundError('SmartResumeSkill', id);
    return skill;
  }

  async create(input: CreateSmartResumeSkillInput, clerkUserId: string) {
    ...
    if (input.skill_id !== undefined) record.skill_id = input.skill_id;
    if (input.category !== undefined) record.category = input.category;
    if (input.proficiency !== undefined) record.proficiency = input.proficiency;
    if (input.years_used !== undefined) record.years_used = input.years_used;
  ...
  async update(id: string, input: UpdateSmartResumeSkillInput, clerkUserId: string) {
    const existing = await this.repository.findById(id);
    if (!existing) throw new NotFoundError('SmartResumeSkill', id);

    await this.assertOwnership(clerkUserId, existing.profile_id);

    const updates: Record<string, any> = {};
    if (input.skill_id !== undefined) updates.skill_id = input.skill_id;
    if (input.name !== undefined) updates.name = input.name;
    if (input.category !== undefined) updates.category = input.category;
    if (input.proficiency !== undefined) updates.proficiency = input.proficiency;
    if (input.years_used !== undefined) updates.years_used = input.years_used;
    if (input.visible_to_matching !== undefined) updates.visible_to_matching = input.visible_to_matching;
    if (input.sort_order !== undefined) updates.sort_order = input.sort_order;

    const updated = await this.repository.update(id, updates);
    if (!updated) throw new NotFoundError('SmartResumeSkill', id);
