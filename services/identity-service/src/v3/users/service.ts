...
export class UserService {
  ...
  /**
   * Resolve the correct Clerk secret key based on which app the request came from.
   * Portal users are in the SPLITS Clerk instance, candidate users in the APP instance.
   */
  private getClerkSecretKey(sourceApp?: SourceApp): string | undefined {
    if (sourceApp === 'candidate') {
      return process.env.APP_CLERK_SECRET_KEY;
    }
    // Default to portal (SPLITS) — most profile operations come from portal
    return process.env.SPLITS_CLERK_SECRET_KEY;
  }
  ...
  async update(id: string, input: UpdateUserInput, clerkUserId: string, sourceApp?: SourceApp) {
    ...
    if (input.name !== undefined) {
      this.syncUserNameToClerk(existing.clerk_user_id, input.name, sourceApp).catch(() => {});
    }
  ...
  private async syncUserNameToClerk(clerkUserId: string, fullName: string, sourceApp?: SourceApp): Promise<void> {
    const secretKey = this.getClerkSecretKey(sourceApp);
    if (!secretKey) return;
  ...
  async updateMe(clerkUserId: string, input: UpdateUserInput, sourceApp?: SourceApp) {
    const user = await this.repository.findByClerkId(clerkUserId);
    if (!user) throw new NotFoundError('User', clerkUserId);
    return this.update(user.id, input, clerkUserId, sourceApp);
  }
  ...
  async updateProfileImage(
    ...
    // Sync to Clerk (best-effort, don't block)
    this.syncProfileImageToClerk(clerkUserId, profileImageData.profile_image_url, sourceApp);
  ...
  async deleteProfileImage(clerkUserId: string, sourceApp?: SourceApp) {
    const user = await this.repository.findByClerkId(clerkUserId);
    if (!user) throw new NotFoundError('User', clerkUserId);
    ...
    // Remove from Clerk (best-effort, don't block)
    this.deleteProfileImageFromClerk(clerkUserId, sourceApp);
  ...
  private async syncProfileImageToClerk(clerkUserId: string, imageUrl: string, sourceApp?: SourceApp): Promise<void> {
    const secretKey = this.getClerkSecretKey(sourceApp);
    if (!secretKey) return;
    ...
    try {
      ...
      await clerkClient.users.updateUserProfileImage(clerkUserId, { file: imageBlob });
      await clerkClient.users.updateUser(clerkUserId, {
        publicMetadata: { profile_image_url: imageUrl },
      });
    } catch {
  ...
  private async deleteProfileImageFromClerk(clerkUserId: string, sourceApp?: SourceApp): Promise<void> {
    const secretKey = this.getClerkSecretKey(sourceApp);
    if (!secretKey) return;
    ...
    try {
      ...
      await clerkClient.users.updateUser(clerkUserId, {
        publicMetadata: { profile_image_url: null },
      });
    } catch {
      // Best-effort — don't throw
    }
