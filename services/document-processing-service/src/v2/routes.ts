...
import { DocumentFilters, DocumentUpdate, PaginationParams } from './shared/types';
...
export async function registerV2Routes(
  ...
  app.get('/v2/documents', async (request: FastifyRequest<{
  ...
  app.get('/v2/documents/:id', async (request: FastifyRequest<{
    Params: { id: string };
  }>, reply: FastifyReply) => {
    const clerkUserId = request.headers['x-clerk-user-id'] as string;
    if (!clerkUserId) {
    ...
    const document = await repository.get(request.params.id, clerkUserId);
    return reply.send({ data: document });
  });

  // UPDATE - Single method handles ALL updates
  app.patch('/v2/documents/:id', async (request: FastifyRequest<{
    Params: { id: string };
    Body: DocumentUpdate;
  }>, reply: FastifyReply) => {
    const clerkUserId = request.headers['x-clerk-user-id'] as string;
    ...
    const updated = await service.update(request.params.id, clerkUserId, request.body);
    return reply.send({ data: updated });
  });

  // STATS - Processing statistics (system endpoint)
  app.get('/v2/documents/stats', async (request: FastifyRequest, reply: FastifyReply) => {
