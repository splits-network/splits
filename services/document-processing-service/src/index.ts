...
async function main() {
  ...
  try {
    ...
    logger.info(`Document processing service HTTP server started on port ${port}`);
    ...
    await channel.assertExchange('splits-network-events', 'topic', { durable: true });
    
    // Ensure queue exists and bind to routing keys
    const queue = 'document-processing-service';
    await channel.assertQueue(queue, { durable: true });
    await channel.bindQueue(queue, 'splits-network-events', 'document.uploaded');
    ...
    logger.info(`Initial processing statistics: ${pendingCount} pending documents`);

    // Process any existing pending documents on startup
    if (pendingCount > 0) {
      logger.info(`Found ${pendingCount} pending documents, starting retroactive processing`);
      ...
      while (remainingPending > 0) {
        ...
        if (remainingPending > 0) {
          ...
          logger.info(`Processed batch, waiting before next batch. Remaining: ${remainingPending}`);
    ...
    logger.error(`Failed to start Document Processing Service: ${error instanceof Error ? error.message : String(error)}`);
    process.exit(1);
  }
}
