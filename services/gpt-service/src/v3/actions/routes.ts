...
import { JobSearchParams, jobSearchQuerySchema, jobIdParamSchema } from './types';

export function registerGptActionRoutes(app: FastifyInstance, supabase: SupabaseClient) {
  ...
  app.get('/api/v3/gpt/jobs/search', {
    schema: { querystring: jobSearchQuerySchema },
  }, async (request, reply) => {
    const result = await service.searchJobs(request.query as JobSearchParams);
    return reply.send({ data: result.data, pagination: result.pagination });
  });

  // GET /api/v3/gpt/jobs/:id
  app.get('/api/v3/gpt/jobs/:id', {
