...
export async function registerMetricRoutes(
    ...
    const repository = new MarketplaceMetricsRepository(
        config.supabaseUrl,
        config.supabaseKey,
    );
    ...
    const service = new MarketplaceMetricsServiceV2(
        repository,
        accessResolver,
        config.eventPublisher,
    );

    app.get("/api/v2/marketplace-metrics", async (request, reply) => {
        try {
            ...
            return reply
                ...
                .send({
                    error: {
                        message:
                            error.message ||
                            "Failed to fetch marketplace metrics",
                    },
                });
        }
    });

    app.get("/api/v2/marketplace-metrics/:id", async (request, reply) => {
        try {
            const { clerkUserId } = requireUserContext(request);
            ...
            return reply
                ...
                .send({
                    error: {
                        message:
                            error.message || "Marketplace metric not found",
                    },
                });
        }
    });

    app.post("/api/v2/marketplace-metrics", async (request, reply) => {
        try {
            const { clerkUserId } = requireUserContext(request);
            ...
            if (
                ...
                return reply.code(400).send({
                    error: {
                        message:
                            "date, totals, activity, and health_score are required",
                    },
                });
            }
            ...
            return reply
                ...
                .send({
                    error: {
                        message:
                            error.message ||
                            "Failed to create marketplace metric",
                    },
                });
        }
    });

    app.patch("/api/v2/marketplace-metrics/:id", async (request, reply) => {
        try {
            const { clerkUserId } = requireUserContext(request);
            const { id } = request.params as { id: string };
            ...
            return reply
                ...
                .send({
                    error: {
                        message:
                            error.message ||
                            "Failed to update marketplace metric",
                    },
                });
        }
    });

    app.delete("/api/v2/marketplace-metrics/:id", async (request, reply) => {
        try {
            const { clerkUserId } = requireUserContext(request);
            const { id } = request.params as { id: string };
            ...
            return reply
                ...
                .send({
                    error: {
                        message:
                            error.message ||
                            "Failed to delete marketplace metric",
                    },
                });
        }
    });
