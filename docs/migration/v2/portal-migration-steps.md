...
- ✅ Documents upload using V2 (`/api/v2/documents`)
...
- Should be: `this.request('/api/v2/jobs')` → `{baseV2}/v2/jobs` → `/api/v2/jobs`
...
| `getJobs()`                | `/jobs`               | `/api/v2/jobs`               | Job listings           |
| `getRoles()`               | `/jobs`               | `/api/v2/jobs`               | Role management        |
| `getJob(id)`               | `/jobs/${id}`         | `/api/v2/jobs/${id}`         | Job details            |
| `createJob()`              | `/jobs`               | `/api/v2/jobs`               | Job creation           |
| `updateJob()`              | `/jobs/${id}`         | `/api/v2/jobs/${id}`         | Job updates            |
| `getApplicationsByJob()`   | `/applications`       | `/api/v2/applications`       | Application listings   |
| `submitCandidate()`        | `/applications`       | `/api/v2/applications`       | Application submission |
| `updateApplicationStage()` | `/applications/${id}` | `/api/v2/applications/${id}` | Stage management       |
...
| Method                        | Current Path          | V2 Path                      | Impact              |
| ----------------------------- | --------------------- | ---------------------------- | ------------------- |
| `getCandidates()`             | `/candidates`         | `/api/v2/candidates`         | Candidate listings  |
| `getRecruiterProfile()`       | `/recruiters/${id}`   | `/api/v2/recruiters/${id}`   | Recruiter profiles  |
| `updateRecruiterProfile()`    | `/recruiters/${id}`   | `/api/v2/recruiters/${id}`   | Profile updates     |
| `getPendingApplications()`    | `/applications`       | `/api/v2/applications`       | Application review  |
| `getApplicationFullDetails()` | `/applications/${id}` | `/api/v2/applications/${id}` | Application details |
...
| Method                                | Current Path            | V2 Path                        | Impact             |
| ------------------------------------- | ----------------------- | ------------------------------ | ------------------ |
| `getMySubscription()`                 | `/subscriptions`        | `/api/v2/subscriptions`        | Billing management |
| `getPlacements()`                     | `/placements`           | `/api/v2/placements`           | Placement tracking |
| `createPlacement()`                   | `/placements`           | `/api/v2/placements`           | Placement creation |
| `getDocument()`                       | `/documents/${id}`      | `/api/v2/documents/${id}`      | Document access    |
| `getDocumentsByEntity()`              | `/documents`            | `/api/v2/documents`            | Document listings  |
| `deleteDocument()`                    | `/documents/${id}`      | `/api/v2/documents/${id}`      | Document deletion  |
| `getRecruiterCandidateRelationship()` | `/recruiter-candidates` | `/api/v2/recruiter-candidates` | Relationship mgmt  |
| `getStats()`                          | `/stats`                | `/api/v2/stats`                | Dashboard stats    |
...
| ------------------------------ | --------------------------------------------- | -------------------------------- | ---------------- |
| `getRecruiterJobs()`           | `/jobs` (V1)                                  | `/api/v2/jobs` (?)               | Has TODO comment |
| `recruiterSubmitApplication()` | `/applications` (V1)                          | `/api/v2/applications` (?)       | Explicit V1 call |
| `requestPreScreen()`           | `/applications/${id}/request-pre-screen` (V1) | `/api/v2/applications/${id}` (?) | Explicit V1 call |
...
For each legacy endpoint, update the path to include `/api/v2/` prefix:
...
async getJobs(params?: JobFilters): Promise<{ data: Job[], pagination?: PaginationResponse }> {
  const response = await this.request('/api/v2/jobs', { params });
