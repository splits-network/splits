...
app.post(
    ...
    async (
        ...
        const candidate =
            await repository.findCandidateByClerkUserId(clerkUserId);
        if (!candidate) {
        ...
        if (application.stage !== "recruiter_proposed") {
            return reply.status(409).send({
                error: {
                    code: "INVALID_STAGE",
                    message: "Application is not in recruiter_proposed stage",
                },
            });
        }
...
app.patch(
    ...
    async (
        ...
        const clerkUserId = request.headers["x-clerk-user-id"] as string;
        const { document_ids, primary_resume_id, pre_screen_answers, notes } =
            request.body;
        ...
        if (!document_ids || document_ids.length === 0 || !primary_resume_id) {
            throw new BadRequestError(
                "document_ids and primary_resume_id are required",
            );
        }
        ...
        if (application.stage !== "draft") {
            return reply.status(409).send({
                error: {
                    code: "INVALID_STAGE",
                    message: "Application must be in draft stage to complete",
                },
            });
        }
...
app.post(
    "/api/applications/:id/accept-proposal",
    {
        preHandler: requireRoles(["candidate"], services),
    },
    async (request, reply) => {
        return atsService().post(
            `/applications/${request.params.id}/accept-proposal`,
            {
                headers: {
...
app.post(
    "/api/applications/:id/decline-proposal",
    {
        preHandler: requireRoles(["candidate"], services),
    },
    async (request, reply) => {
        return atsService().post(
            `/applications/${request.params.id}/decline-proposal`,
            {
                headers: {
                    "x-clerk-user-id": request.auth.clerkUserId,
                    "x-user-role": "candidate",
...
app.patch(
    ...
    async (request, reply) => {
        return atsService().patch(
            `/applications/${request.params.id}/complete`,
            {
                headers: {
                    "x-clerk-user-id": request.auth.clerkUserId,
                    "x-user-role": "candidate",
    ...
    return (
        <div className="alert alert-info mb-6 shadow-lg">
            <div className="flex-1">
                ...
                <div>
                    <h3 className="font-bold text-lg mb-1">
                        Job Opportunity from {recruiter.first_name}{" "}
                        {recruiter.last_name}
                    </h3>
                    <p className="text-sm">
                            ...
                            <div className="flex items-start gap-2">
                                ...
                                <div className="flex-1">
                                    <div className="text-xs text-base-content/60 mb-1">
                                        Why {recruiter.first_name} thinks this
                                        is a good match:
                                    </div>
            ...
            <div className="flex-none flex flex-col sm:flex-row gap-2">
                <button onClick={onDecline} className="btn btn-ghost gap-2">
                    <i className="fa-duotone fa-regular fa-times"></i>
                    Not Interested
                </button>
                <button onClick={onAccept} className="btn btn-primary gap-2">
            ...
            <div className="modal-box">
                ...
                <div className="alert mb-4">
                    ...
                    <span>
                        ...
                        <strong>{jobTitle}</strong>? Your recruiter will be
                    ...
                    <fieldset className="fieldset">
                        ...
                        <label className="label">
                            <span className="label-text-alt">
                                This will be shared with your recruiter to help
                                them understand your preferences.
                            </span>
                        </label>
                    </fieldset>
                </div>
...
export default function ProposalResponseWizard({
    ...
    const handleNext = () => {
        ...
        if (currentStep === 1 && !primaryResumeId) {
            setError(
                "Please upload and select a primary resume before continuing",
            );
            return;
        }
        ...
        try {
            ...
            await axios.patch(
                `${apiUrl}/applications/${applicationId}/complete`,
                {
                    ...
                    pre_screen_answers: Object.entries(answers).map(
                        ([questionId, answer]) => ({
