...
export class ExampleRepository {
    async findItems(
        ...
        let query = this.supabase.from("items").select("*", { count: "exact" });
        ...
        if (accessContext.role === "candidate") {
            ...
            const assignments = await this.getRecruiterAssignments(
        ...
        const sortOrder =
            params.sort_order?.toLowerCase() === "asc" ? true : false;
...
for (const key of Object.keys(filters)) {
    ...
    if (value !== undefined && value !== null) {
        ...
        if (
            ["search", "scope", "created_after", "created_before"].includes(key)
        ) {
            continue;
        }
...
if (filters.scope === "mine" && accessContext.recruiterId) {
    ...
    query = await this.applyRecruiterScopeFilter(
        query,
...
describe("CandidateRepository.findCandidates", () => {
    ...
    it("should enforce role-based access", async () => {
        const candidateResult = await repository.findCandidates(
            "candidate_clerk_id",
            {},
        );
        const recruiterResult = await repository.findCandidates(
            "recruiter_clerk_id",
            {},
        );
        ...
        expect(
            candidateResult.data.every(
                (c) => c.user_id === "candidate_user_id",
            ),
        ).toBe(true);
